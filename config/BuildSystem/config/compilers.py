...
''' + 'void '+self.mangleFortranFunction('f90ptrtest')+'''(void* a1, void* a2,void* a3, void* i, void* p1 ,void* p2, void* p3)
{
  ...
  def checkC99Flag(self):
    ...
    currentflags = getattr(self, self.setCompilers.getCompilerFlagsArg(compilerOnly = 1))
    print currentflags
    if currentflags.find('-std=c89') > -1:
      ''' Some C compilers allow having -std=c89 and -std=c99 flags at the same time and then use c99''' 
      self.logWrite(self.setCompilers.restoreLog())
      self.logWrite('Found -std=c89 in C flags, therefor skipping checking of C99 flags\n')
      return
    restoredlog = 0
    flags_to_try = ['','-std=c99','-std=gnu99','-std=c11''-std=gnu11','-c99']
    for flag in flags_to_try:
      if self.setCompilers.checkCompilerFlag(flag, includes, body):
        self.setCompilers.insertCompilerFlag(flag,compilerOnly = 1)
        self.c99flag = flag
        self.logWrite(self.setCompilers.restoreLog())
        restoredlog = 1
