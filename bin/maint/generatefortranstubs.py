...
def processf90interfaces(petscdir,verbose):
  ...
  for mansec in os.listdir(os.path.join(petscdir,'include','finclude','ftn-auto')):
    ...
    mansecdir = os.path.join(petscdir,'include','finclude','ftn-auto',mansec)
    if os.path.isdir(mansecdir):
      mansec = mansec[:-7]
      f90inc = os.path.join(petscdir,'include','finclude','ftn-auto','petsc'+mansec+'.h90')
      fd = open(f90inc,'w')
      for sfile in os.listdir(mansecdir):
        if verbose: print '  Copying in '+sfile
        fdr = open(os.path.join(mansecdir,sfile))
        txt = fdr.read()
        fd.write(txt)
        fdr.close()
      fd.close()
      shutil.rmtree(mansecdir)
  FixDir(petscdir,os.path.join(petscdir,'include','finclude','ftn-auto'),verbose)
  return

def main(petscdir,bfort,dir,verbose):
  # cleanup include/finclude/ftn-auto
  fincauto = os.path.join(petscdir,'include','finclude','ftn-auto')
  if os.path.isdir(fincauto): shutil.rmtree(fincauto)
  # now process petsc sources
