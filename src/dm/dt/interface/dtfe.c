...
PetscErrorCode PetscSpaceEvaluate_Subspace(PetscSpace sp, PetscInt npoints, const PetscReal points[], PetscReal B[], PetscReal D[], PetscReal H[])
{
  ...
  ierr = DMGetWorkArray(sp->dm,npoints*origDim,PETSC_REAL,&inpoints);CHKERRQ(ierr);
  for (i = 0; i < npoints; i++) {
    if (subsp->x) {
  ...
  if (B) {
    ierr = DMGetWorkArray(sp->dm,npoints*origNb*origNc,PETSC_REAL,&inB);CHKERRQ(ierr);
  }
  if (D) {
    ierr = DMGetWorkArray(sp->dm,npoints*origNb*origNc*origDim,PETSC_REAL,&inD);CHKERRQ(ierr);
  }
  if (H) {
    ierr = DMGetWorkArray(sp->dm,npoints*origNb*origNc*origDim*origDim,PETSC_REAL,&inH);CHKERRQ(ierr);
  }
  ierr = PetscSpaceEvaluate(origsp,npoints,inpoints,inB,inD,inH);CHKERRQ(ierr);
  if (H) {
    PetscReal *phi, *psi;

    ierr = DMGetWorkArray(sp->dm,origNc*origDim*origDim,PETSC_REAL,&phi);CHKERRQ(ierr);
    ierr = DMGetWorkArray(sp->dm,origNc*subDim*subDim,PETSC_REAL,&psi);CHKERRQ(ierr);
    ...
    ierr = DMRestoreWorkArray(sp->dm,subNc*origDim,PETSC_REAL,&psi);CHKERRQ(ierr);
    ierr = DMRestoreWorkArray(sp->dm,origNc*origDim,PETSC_REAL,&phi);CHKERRQ(ierr);
    ierr = DMRestoreWorkArray(sp->dm,npoints*origNb*origNc*origDim,PETSC_REAL,&inH);CHKERRQ(ierr);
  }
  if (D) {
    PetscReal *phi, *psi;

    ierr = DMGetWorkArray(sp->dm,origNc*origDim,PETSC_REAL,&phi);CHKERRQ(ierr);
    ierr = DMGetWorkArray(sp->dm,origNc*subDim,PETSC_REAL,&psi);CHKERRQ(ierr);
    for (i = 0; i < npoints * subNb * subNc * subDim; i++) D[i] = 0.0;
    for (i = 0; i < subNb; i++) {
      const PetscReal *subq = &subsp->Q[i * origNb];
    ...
    ierr = DMRestoreWorkArray(sp->dm,subNc*origDim,PETSC_REAL,&psi);CHKERRQ(ierr);
    ierr = DMRestoreWorkArray(sp->dm,origNc*origDim,PETSC_REAL,&phi);CHKERRQ(ierr);
    ierr = DMRestoreWorkArray(sp->dm,npoints*origNb*origNc*origDim,PETSC_REAL,&inD);CHKERRQ(ierr);
  }
  if (B) {
    PetscReal *phi;

    ierr = DMGetWorkArray(sp->dm,origNc,PETSC_REAL,&phi);CHKERRQ(ierr);
    if (subsp->u) {
    ...
    ierr = DMRestoreWorkArray(sp->dm,origNc,PETSC_REAL,&phi);CHKERRQ(ierr);
    ierr = DMRestoreWorkArray(sp->dm,npoints*origNb*origNc,PETSC_REAL,&inB);CHKERRQ(ierr);
  }
  ierr = DMRestoreWorkArray(sp->dm,npoints*origDim,PETSC_REAL,&inpoints);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
                                      ...
                                      const PetscScalar coefficients[], PetscDS probAux, const PetscScalar coefficientsAux[], PetscReal integral[])
{
  ...
  PetscInt       dim, Nf, dE, Np, NfAux = 0, totDim, totDimAux = 0, cOffset = 0, cOffsetAux = 0, e;
  PetscBool      isAffine;
  ...
  for (e = 0; e < Ne; ++e) {
    ...
    for (q = 0; q < Nq; ++q) {
      ...
      obj_func(dim, Nf, NfAux, uOff, uOff_x, u, NULL, u_x, aOff, aOff_x, a, NULL, a_x, 0.0, v, &integrand);
                                              ...
                                              const PetscScalar coefficients[], const PetscScalar coefficients_t[], PetscDS probAux, const PetscScalar coefficientsAux[], PetscReal t, PetscScalar elemVec[])
{
  ...
  for (e = 0; e < Ne; ++e) {
    ...
    for (q = 0; q < Nq; ++q) {
      ...
      if (probAux) EvaluateFieldJets(dim, NfAux, NbAux, NcAux, q, BAux, DAux, refSpaceDerAux, invJ, &coefficientsAux[cOffsetAux], NULL, a, a_x, NULL);
      if (f0_func) f0_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, v, &f0[q*NcI]);
      if (f1_func) f1_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, v, refSpaceDer);
                                                ...
                                                const PetscScalar coefficients[], const PetscScalar coefficients_t[], PetscDS probAux, const PetscScalar coefficientsAux[], PetscReal t, PetscScalar elemVec[])
{
  ...
  for (e = 0; e < Ne; ++e) {
    ...
    for (q = 0; q < Nq; ++q) {
      ...
      if (f0_func) f0_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, v, n, &f0[q*NcI]);
      if (f1_func) f1_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, v, n, refSpaceDer);
      TransformF(dim, NcI, q, invJ, detJ, quadWeights, refSpaceDer, f0_func ? f0 : NULL, f1_func ? f1 : NULL);
    }
                                              ...
                                              const PetscScalar coefficients[], const PetscScalar coefficients_t[], PetscDS probAux, const PetscScalar coefficientsAux[], PetscReal t, PetscReal u_tshift, PetscScalar elemMat[])
{
  ...
  for (e = 0; e < Ne; ++e) {
    ...
    for (q = 0; q < Nq; ++q) {
      ...
      if (g0_func) {
        ierr = PetscMemzero(g0, NcI*NcJ * sizeof(PetscScalar));CHKERRQ(ierr);
        g0_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, u_tshift, v, g0);
        for (c = 0; c < NcI*NcJ; ++c) g0[c] *= w;
      }
      if (g1_func) {
        PetscInt d, d2;
        ierr = PetscMemzero(refSpaceDer, NcI*NcJ*dim * sizeof(PetscScalar));CHKERRQ(ierr);
        g1_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, u_tshift, v, refSpaceDer);
        for (fc = 0; fc < NcI; ++fc) {
          for (gc = 0; gc < NcJ; ++gc) {
            for (d = 0; d < dim; ++d) {
      ...
      if (g2_func) {
        PetscInt d, d2;
        ierr = PetscMemzero(refSpaceDer, NcI*NcJ*dim * sizeof(PetscScalar));CHKERRQ(ierr);
        g2_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, u_tshift, v, refSpaceDer);
        for (fc = 0; fc < NcI; ++fc) {
          for (gc = 0; gc < NcJ; ++gc) {
            for (d = 0; d < dim; ++d) {
      ...
      if (g3_func) {
        ...
        g3_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, u_tshift, v, refSpaceDer);
        for (fc = 0; fc < NcI; ++fc) {
          for (gc = 0; gc < NcJ; ++gc) {
            for (d = 0; d < dim; ++d) {
              for (dp = 0; dp < dim; ++dp) {
                                                ...
                                                const PetscScalar coefficients[], const PetscScalar coefficients_t[], PetscDS probAux, const PetscScalar coefficientsAux[], PetscReal t, PetscReal u_tshift, PetscScalar elemMat[])
{
  ...
  for (e = 0; e < Ne; ++e) {
    ...
    for (q = 0; q < Nq; ++q) {
      ...
      if (g0_func) {
        ierr = PetscMemzero(g0, NcI*NcJ * sizeof(PetscScalar));CHKERRQ(ierr);
        g0_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, u_tshift, v, n, g0);
        for (c = 0; c < NcI*NcJ; ++c) g0[c] *= w;
      }
      if (g1_func) {
        PetscInt d, d2;
        ierr = PetscMemzero(refSpaceDer, NcI*NcJ*dim * sizeof(PetscScalar));CHKERRQ(ierr);
        g1_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, u_tshift, v, n, refSpaceDer);
        for (fc = 0; fc < NcI; ++fc) {
          for (gc = 0; gc < NcJ; ++gc) {
            for (d = 0; d < dim; ++d) {
              g1[(fc*NcJ+gc)*dim+d] = 0.0;
      ...
      if (g2_func) {
        PetscInt d, d2;
        ierr = PetscMemzero(refSpaceDer, NcI*NcJ*dim * sizeof(PetscScalar));CHKERRQ(ierr);
        g2_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, u_tshift, v, n, refSpaceDer);
        for (fc = 0; fc < NcI; ++fc) {
          for (gc = 0; gc < NcJ; ++gc) {
            for (d = 0; d < dim; ++d) {
              g2[(fc*NcJ+gc)*dim+d] = 0.0;
      ...
      if (g3_func) {
        PetscInt d, d2, dp, d3;
        ierr = PetscMemzero(refSpaceDer, NcI*NcJ*dim*dim * sizeof(PetscScalar));CHKERRQ(ierr);
        g3_func(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, NULL, a_x, t, u_tshift, v, n, refSpaceDer);
        for (fc = 0; fc < NcI; ++fc) {
          for (gc = 0; gc < NcJ; ++gc) {
            for (d = 0; d < dim; ++d) {
              for (dp = 0; dp < dim; ++dp) {
