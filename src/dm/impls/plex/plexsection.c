...
static PetscErrorCode DMPlexCreateSectionInitial(DM dm, PetscInt dim, PetscInt numFields,const PetscInt numComp[],const PetscInt numDof[], DMLabel label[], PetscSection *section)
{
  DMLabel        depthLabel;
  PetscInt      *pMax;
...
@*/
PetscErrorCode DMPlexCreateSection(DM dm, PetscInt dim, PetscInt numFields,const PetscInt numComp[],const PetscInt numDof[], DMLabel label[], PetscInt numBC,const PetscInt bcField[], const IS bcComps[], const IS bcPoints[], IS perm, PetscSection *section)
{
  PetscSection   aSec;
  PetscErrorCode ierr;

  PetscFunctionBegin;
  ierr = DMPlexCreateSectionInitial(dm, dim, numFields, numComp, numDof, label, section);CHKERRQ(ierr);
...
PetscErrorCode DMCreateDefaultSection_Plex(DM dm)
{
  ...
  ierr = PetscCalloc3(Nf,&numComp,Nf*(dim+1),&numDof,Nf,&label);CHKERRQ(ierr);
  for (f = 0; f < Nf; ++f) {
    PetscObject obj;

    ierr = PetscDSGetDiscretization(prob, f, &obj);CHKERRQ(ierr);
    ierr = PetscDSGetLabel(prob, f, &label[f]);CHKERRQ(ierr);
    if (isFE[f]) {
  ...
  ierr = DMPlexCreateSection(dm, dim, Nf, numComp, numDof, label, numBC, bcFields, bcComps, bcPoints, NULL, &section);CHKERRQ(ierr);
  for (f = 0; f < Nf; ++f) {
    PetscFE     fe;
    const char *name;
  ...
  ierr = PetscFree3(numComp,numDof,label);CHKERRQ(ierr);
  ierr = PetscFree(isFE);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
