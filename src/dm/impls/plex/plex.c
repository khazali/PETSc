...
#undef __FUNCT__
#define __FUNCT__ "DMPlexConstraintsModifyMat"
static PetscErrorCode DMPlexConstraintsModifyMat(DM dm, PetscSection section, PetscInt numPoints, PetscInt numIndices, const PetscInt points[], const PetscScalar values[], PetscInt *outNumPoints, PetscInt *outNumIndices, PetscInt *outPoints[], PetscScalar *outValues[], PetscInt offsets[])
{
  Mat             cMat;
...
PetscErrorCode DMPlexMatSetClosure(DM dm, PetscSection section, PetscSection globalSection, Mat A, PetscInt point, const PetscScalar values[], InsertMode mode)
{
  ...
  ierr = DMPlexConstraintsModifyMat(dm,section,numPoints,numIndices,points,values,&newNumPoints,&newNumIndices,&newPoints,&newValues,offsets);CHKERRQ(ierr);
  if (newNumPoints) {
    if (!clPoints) {
...
#undef __FUNCT__
#define __FUNCT__ "MatMult_PlexProjectConstraintsNormal"
PetscErrorCode MatMult_PlexProjectConstraintsNormal(Mat CtC, Vec x, Vec y)
{
  DM                    dm;
  Vec                   local, mask;
  ...
  ierr = VecPointwiseMult(local,mask,local);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "DMPlexProjectConstraints_Internal"
/* Solve the normal equation for the constraints: x is local and y is global */
PetscErrorCode DMPlexProjectConstraints_Internal(DM dm, Vec x, Vec y)
{
  Mat                   cMat, CtC;
  ...
  ierr = MatShellSetOperation(CtC,MATOP_MULT,(void(*)(void))MatMult_PlexProjectConstraintsNormal);CHKERRQ(ierr);
  ...
  ierr = VecPointwiseMult(x,mask,x);CHKERRQ(ierr);
