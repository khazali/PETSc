    ...
    for (p = pStartC; p < pEndC; p++) {
      ...
      if (maxChildId >= 0) { /* build an identity matrix, apply matrix constraints on the right */
        ...
        if (numFields) {
          ...
          DMPlexGetIndicesPointFields_Internal(cSec,p,cOff,offsetsCopy,PETSC_TRUE,rowIndices);CHKERRQ(ierr);
          for (a = 0; a < aDof; a++) {
            PetscInt anchor = anchors[a + aOff], lOff;
            ierr = PetscSectionGetOffset(localCoarse,anchor,&lOff);CHKERRQ(ierr);
            DMPlexGetIndicesPointFields_Internal(localCoarse,anchor,lOff,newOffsetsCopy,PETSC_TRUE,colIndices);CHKERRQ(ierr);
          }
        }
        else {
          DMPlexGetIndicesPoint_Internal(cSec,p,cOff,offsetsCopy,PETSC_TRUE,rowIndices);CHKERRQ(ierr);
          for (a = 0; a < aDof; a++) {
            PetscInt anchor = anchors[a + aOff], lOff;
            ierr = PetscSectionGetOffset(localCoarse,anchor,&lOff);CHKERRQ(ierr);
            DMPlexGetIndicesPoint_Internal(localCoarse,anchor,lOff,newOffsetsCopy,PETSC_TRUE,colIndices);CHKERRQ(ierr);
          }
        }
        if (numFields) {
          PetscInt count, a;
          ...
          for (a = 0; a < aDof; a++) {
            ...
            DMPlexGetIndicesPointFields_Internal(localCoarse,anchor,gOff < 0 ? -(gOff + 1) : gOff,newOffsets,PETSC_FALSE,pInd);CHKERRQ(ierr);
          }
        }
        else {
          PetscInt a;
          ...
          for (a = 0; a < aDof; a++) {
            ...
            PetscInt gOff;
            ierr = PetscSectionGetOffset(globalCoarse,anchor,&gOff);CHKERRQ(ierr);
            DMPlexGetIndicesPoint_Internal(localCoarse,anchor,gOff < 0 ? -(gOff + 1) : gOff,newOffsets,PETSC_FALSE,pInd);CHKERRQ(ierr);
          }
        }
        ...
        if (numFields) {
          ...
          DMPlexGetIndicesPointFields_Internal(localCoarse,p,gOff < 0 ? -(gOff + 1) : gOff,offsets,PETSC_FALSE,pInd);CHKERRQ(ierr);
        }
        else {
          DMPlexGetIndicesPoint_Internal(localCoarse,p,gOff < 0 ? -(gOff + 1) : gOff,offsets,PETSC_FALSE,pInd);CHKERRQ(ierr);
        }
      }
    }
    ...
    for (l = 0, offset = 0; l < nleaves; l++) {
      ...
      if ((dof - cdof) > 0) {
        ...
        if (numFields) {
          ...
          DMPlexGetIndicesPointFields_Internal(localFine,p,gOff < 0 ? -(gOff + 1) : gOff,offsets,PETSC_FALSE,pInd);CHKERRQ(ierr);
        }
        else {
          DMPlexGetIndicesPoint_Internal(localFine,p,gOff < 0 ? -(gOff + 1) : gOff,offsets,PETSC_FALSE,pInd);CHKERRQ(ierr);
        }
...
PetscErrorCode DMPlexComputeInjectorTree(DM coarse, DM fine, PetscSF coarseToFine, PetscInt *childIds, Mat mat)
{
  ...
  for (p = pStartC; p < pEndC; p++) {
    ...
    if (numFields) {
      ...
      DMPlexGetIndicesPointFields_Internal(localCoarse,p,gOff < 0 ? -(gOff + 1) : gOff,offsetsCopy,PETSC_FALSE,parentIndices);CHKERRQ(ierr);
    }
    else {
      DMPlexGetIndicesPoint_Internal(localCoarse,p,gOff < 0 ? -(gOff + 1) : gOff,offsetsCopy,PETSC_FALSE,parentIndices);CHKERRQ(ierr);
