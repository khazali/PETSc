...
\title{Algorithms and software for extreme-scale solvers in XGC\footnote{White paper submitted to R. Laviolette at DOE-ASCR}}
\author{Mark F. Adams}
\nolinenumbers
\begin{document}
\maketitle
...
{ \bf
We propose to address the challenges of extreme-scale solvers on emerging architectures for anisotropic particle-in-cell (PIC) methods in general and XGC in particular with new field solver algorithms and data decompositions deployed in the PETSc numerical library and driven by the extreme-scale tokamak PIC code X2.
}
\vskip .1cm 
{ \noindent
...
\noindent
Execution period: FY2016-17

\noindent
Other personnel: M. Knepley, T. Issac%, A. Christlieb and J. Verboncoeur
}
\vskip .2cm 
...
For instance, some of these methods require 3D magnetohydrodynamics solvers, which are more demanding than the current 2D finite element Poisson solvers, because the 3D solvers are on stretched grids and the 2D solvers are on fairly isotropic grids.
Magnetohydrodynamics solvers are very challenging in their own right even on structured isotropic grids \cite{Chacon08,Adams-10a}.
XGC uses unstructured stretched grids, to reduce the memory footprint because the full grid data is replicated in each address space.
The replicated data problem is also being addressed by the RPI team and we have experience in addressing this problem in the predecessor to XGC, GTC \cite{Adams-07a}.
Highly stretched grids are not accurate if the solution is not carefully aligned with the mesh, and are difficult to solve efficiently with iterative equation solvers, i.e., algebraic multigrid (AMG).
...
We propose to address these challenges by redesigning the solver algorithms and data partitioning and algorithms (data models) for extreme-scale tokamak PIC methods.
...
GMG is not widely used because it is difficult to deploy flexible and efficient GMG equation solvers in a library, to amortize their cost of development, because they are inherently tied to the meshing and discretizations.
...
We propose to continue, and accelerate, the development PICell solvers specialized for the particular demands of XGC.
...
This work leverages projects in the ASCR base program, the DOE FASTMath institute, and an Intel Parallel Processing Center (IPPC), which funds Dr. Issac.
...
Professor Matthew Knepley at Rice University (\$75,000 per year) will work on the PETSc development and Dr. Mark Adams (\$75,000 per year) will work on the X2 development.
The following tasks are proposed for this project by Professor Knepley's team [K], including Dr. Issac, and Dr. Adams' team [A].
\vskip .1cm 
\begin{itemize}
\item Core PIC functionality in DMPICell and add to X2 [A,K: FY16 Q4]:
\begin{enumerate}
...
\item Algebraic multigrid Poisson equation solver with non-constant material coefficients 
\end{enumerate}
...
\end{itemize}
\item H(curl) finite elements for curl operators [K: FY17 Q1]
\item Start adiabatic electron, electrostatic, gyrokinetic capability [A: FY17 Q2]
\begin{itemize}
\item Begin investigating accuracy requirements in grid methods (e.g., gradients) 
\end{itemize}
\item $B_0$ point interpolation [A: FY17 Q2]
\item Ghost cells and particle deposition communication, as needed, for gyrokinetics, and begin optimizing data models [A,K: FY17 Q2]
\item Begin to develop performance models with physics to prepare for particular exa-scale physics campaigns [A,K: FY17 Q2]
\item Q2 and high order H(curl) finite elements (optional) [K: FY17 Q3]
\item Improve visualization with combined particles and grid [A,K: FY17 Q3]
\item Finite particle form factors or density smoothing [A: FY17 Q3]
\item Fast geometric multigrid Poisson equation solver [A: FY17 Q4]
\item Functional adiabatic electron, electrostatic, (gyrokinetic) core tokamak capability [A,K: FY17 Q4]
\end{itemize}
\vskip .1cm 
We intend to seek funding for a second year for the following work.
\vskip .1cm 
\begin{itemize}
\item Begin working with RPI team in evaluating data models and performance models as appropriate [A: FY18 Q1]
\item Begin full geometry (ITER), with separatrix capability [A: FY18 Q1]
\item Add high order explicit time integrators [A: FY18 Q1]
...
\item Full geometry (ITER), with separatrix capability [A: FY18 Q2]
\item Begin code to code verification with XGC [A: FY18 Q3]
\item Continue extending model capabilities, verifying and optimizing code, and optimizing data models [A,K: FY18 Q4]
\item Convergence studies verification and code to code verification with XGC [A: FY18 Q4]
\end{itemize}
%\pagebreak
...
We propose to support extreme-scale PIC simulations on emerging architectures in PETSc to enable XGC to continue its preeminence for decades to come.
...
In addition to the PETSc library work, this document describes a tokamak PIC code, X2 (\S\ref{sec:x2}).
...
The applied math, engineering, and physics communities have developed powerful discretization and solver methods for extreme-scale computing, but more modern methods are not widely used in the fusion PIC community.
For instance, XGC uses second order accurate finite differencing in the parallel direction and second order accurate linear finite elements in the perpendicular plane, which were in wide spread use by the 1960s.
...
Commercial-grade software engineering takes both skill and time that is not available to most academic physics code projects and is not seen as worth to cost due to their academic reward structure.
PETSc employs commercial-grade software engineering processes and some of our users, even those that work in academia, follow our practices because in the long term it leads to higher productivity. 
This project provides fusion PIC codes with solid infrastructure on which to build their physics, to allow for the full use of their theoretical expertise without the demands of implementing, verifying, and maintaining the increasing complex software stack required to effectively use modern hardware.
...
\item fast linear and nonlinear equation solvers for Poisson and Ampere's law solvers, and implicit PIC methods (magnetohydrodymaic);
...
\begin{itemize}
\item conservative discretizations (FV and DG) are useful for flux surface averaging projections;
\end{itemize}
...
The development proposed herein takes place in a branch in the PETSc repository (mark/feature-picell) and a driver application named X2 (DMPICell src/dm/impls/picell/examples/tutorial/ex1.c).
...
This is critical to continuously verify the code and for an effective develop, debug, and verify code cycle.
...
The mesh in Figure \ref{fig:cross} (right) is generated with seven (high order) Q2 (high order H(curl) finite elements) elements per plane on a coarse grid and is refined three times in this figure.
...
Additionally, the solutions of say the Poisson solve in fusion applications, with turbulence, are smooth at the scale of the (known) Larmor radius and, in fact, physicists often smooth the results of low order Poisson solvers to reduce high frequency components.
