...
{ \bf
We propose to address the challenges of extreme-scale solvers on emerging architectures for anisotropic particle-in-cell (PIC) methods in general and XGC in particular with new field solver algorithms and data decompositions deployed in the PETSc numerical library and driven by an extreme-scale tokamak PIC code X2.
}
\vskip .1cm 
{ \noindent
...
XGC currently uses highly stretched grids, required to reduce the memory footprint because the full grid data is replicated in each address space \cite{Adams-07a}.
