...
/* u := (x^4-x^2)*(y^4-y^2)*(z^4-z^2) with alpha(x) = -1 or -(0.1+x+2y) */
...
/* u := (x^4-2x^3+x^2)*(y^4-2y^3+y^2)*(z^4-2z^3+z^2) */
#undef __FUNCT__
#define __FUNCT__ "PhysicsSolution_x4_x3_x2"
static PetscErrorCode PhysicsSolution_x4_x3_x2(Model mod,PetscReal time,const PetscReal *x,PetscScalar *u,void *ctx)
{
  PetscScalar r;
  PetscScalar e[MAX_DIM];
  PetscInt d;
  ...
  PetscFunctionBeginUser;
  for (d = 0; d < mod->dim; ++d) e[d] = x[d]*x[d];
  for (d = 0; d < mod->dim; ++d) e[d] *= e[d] - 2*x[d] + 1;
  for (r = 1.0, d = 0; d < mod->dim; ++d) r *= e[d];
  *u = r;
  PetscFunctionReturn(0);
}

/* RHS: -Lap((x^4-2x^3+x^2)*(y^4-2y^3+y^2)*(z^4-2z^3+z^2)) */
#undef __FUNCT__
#define __FUNCT__ "PhysicsSource_Lap_x4_x3_x2"
static PetscErrorCode PhysicsSource_Lap_x4_x3_x2(Model mod,PetscReal time,const PetscReal *x,PetscScalar *rhs,void *ctx)
{
  PetscScalar r;
  PetscScalar x2[MAX_DIM]; /* = x*x; */
  PetscScalar a[MAX_DIM]; /* = x2*(x2-2x+1) */
  PetscScalar b[MAX_DIM]; /* = 12*x2-12x+2 */
  PetscInt d;

  PetscFunctionBeginUser;
  for (d = 0; d < mod->dim; ++d) x2[d] = x[d]*x[d];
  for (d = 0; d < mod->dim; ++d)  a[d] = x2[d]*(x2[d]-2*x[d]+1);
  for (d = 0; d < mod->dim; ++d)  b[d] = 12*x2[d]-12*x[d]+2;
...
static PetscErrorCode PhysicsCreate_Lap(Model mod,Physics phys)
{
  ...
  lap = (Physics_Lap*)phys->data;
  {
    ...
    if (1) {
      ...
      ierr = ModelSolutionSetDefault(mod,PhysicsSolution_x4_x3_x2,phys);CHKERRQ(ierr);
      ierr = ModelSourceSetDefault(mod,PhysicsSource_Lap_x4_x3_x2,phys);CHKERRQ(ierr);
      ierr = ModelAlphaSetDefault(mod,PhysicsAlpha_n1,phys);CHKERRQ(ierr);
    }
...
PetscErrorCode RHSFunctionLap(TS ts,PetscReal t,Vec globalin,Vec globalout,void *ctx)
{
  ...
  if (s_useBoxStencil) { /* Mehrstellen baby -- does not work because of low order BCs (probably), but does help constants. */
    ...
    ierr = VecGetArrayRead(user->facegeom,&facegeom);CHKERRQ(ierr);
    ierr = VecGetDM(user->facegeom,&dmFace);CHKERRQ(ierr);
    ierr = PetscMalloc1((cEnd-cStart), &uu);CHKERRQ(ierr);
    for (c = cStart; c < cEnd; ++c) {
