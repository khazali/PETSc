...
/*
  ...
  This follows Hairer/Lubich/Wanner  ch XIII example 10.1
  ...
  Note that this problem actually admits a special explicit algorithm (detailed in the book). Here we just use it as a test for 
  the implicit parts of a symplectic Euler or St\"ormer Verlet method. 

  This is an interesting problem. It's also ripe for using an exponential or HMM integrator, as it's highly oscillatory.

  Another good test problem might be the dumbell spacecraft from Cohen 2005
...
int main(int argc, char* argv[])
{
  ...
  /* Note that with the current implementation of TSSYMPEULER, there is no need to define a Jacobian for the Q slot since this is always the 'explicit' part*/
