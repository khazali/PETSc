...
PetscErrorCode ConstructGeometry(DM dm, Vec *facegeom, Vec *cellgeom, User user)
{
  ...
  for (f = fStart; f < fEnd; ++f) {
    ...
    if (ghost >= 0) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"have a ghost face %d",ghost);
...
static PetscErrorCode MonitorVTK(TS ts,PetscInt stepnum,PetscReal time,Vec X,void *ctx)
{
  ...
  if (user->model->functionalCall && ((stepnum == -1) ^ (stepnum % user->vtkInterval == 0))) {
    ...
    ierr = PetscSNPrintf(filename,sizeof filename,"ex33-%03D-u.vtu",stepnum);CHKERRQ(ierr);
    ...
    ierr = PetscSNPrintf(filename,sizeof filename,"ex33-%03D-%s.vtu",stepnum,flink->name);CHKERRQ(ierr);
    ierr = OutputVTK(dm,filename,&viewer);CHKERRQ(ierr);
...
static PetscErrorCode ApplyLaplacianLocal(DM dm,DM dmFace,DM dmCell,PetscReal time,Vec locU,Vec F,User user)
{
  ...
  for (face = fStart; face < fEnd; ++face) {
    ...
    else SETERRQ1(PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONGSTATE,"1) face with %d cells",nC);
    ...
    if (s_useBoxStencil) { /* use a box, not star, stencil */
      ...
      for (s0=0;s0<2;s0++) {
        const FaceGeom *fg1;
        PetscInt numFaces0,f,cell0,amghost=0;
        const PetscInt *faces;
        cell0 = cells0[s0];
        DMPlexGetConeSize(dm, cell0, &numFaces0);
        if (numFaces0!=2*DIM) {
          if (numFaces0!=1) SETERRQ1(PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONGSTATE,"1) cell with %d faces",numFaces0);
          cell0 = cells0[(s0+1)%2];
          ierr = DMPlexGetConeSize(dm, cell0, &numFaces0);CHKERRQ(ierr);
          if (numFaces0!=2*DIM) SETERRQ1(PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONGSTATE,"1) cell with %d faces",numFaces0);
          amghost=1;
        }
        DMPlexGetCone(dm, cell0, &faces);
        for (f = 0; f < numFaces0; ++f) {
          const PetscInt face = faces[f];
          ierr = DMPlexPointLocalRead(dmFace,face,facegeom,&fg1);CHKERRQ(ierr);
          for (i=0,dirs[1]=-1;i<DIM;++i) if (fg1->normal[i] != 0.0 && i != dirs[0]) dirs[1]=i;
          if (dirs[1]!=-1) { /* we have a perp face  */
            PetscInt numFaces1,nC,cell1;
            const PetscInt *cells1;
            const PetscScalar *pu;
            ierr = DMPlexGetSupportSize(dm, face, &nC);CHKERRQ(ierr);
            ierr = DMPlexGetSupport(dm, face, &cells1);CHKERRQ(ierr);
            if (nC==2) {
              /* get cell that is not the source cell */
              if (cells1[0] == cell0) cell1 = cells1[1];
              else if (cells1[1] == cell0) cell1 = cells1[0];
            ...
            else SETERRQ1(PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONGSTATE,"2) face with %d cells -- corner???",nC);
            DMPlexGetConeSize(dm, cell1, &numFaces1);
            if (numFaces1!=2*DIM) {
              if (numFaces1!=1) SETERRQ1(PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONGSTATE,"1) cell with %d faces",numFaces1);
              cell0 = cells0[(s0+1)%2];
              ierr = DMPlexGetConeSize(dm, cell0, &numFaces0);CHKERRQ(ierr);
              if (numFaces0!=2*DIM) SETERRQ1(PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONGSTATE,"1) cell with %d faces",numFaces0);
              amghost=1;
...
PetscErrorCode RHSFunctionLap(TS ts,PetscReal t,Vec globalin,Vec globalout,void *ctx)
{
  ...
  if (s_useBoxStencil) { /* Mehrstellen baby -- does not work because of low order BCs (probably), but does help constants. */
    ...
    ierr = DMGetLocalVector(dm,&locRHS);CHKERRQ(ierr);
    ierr = DMGlobalToLocalBegin(dm,globalout,INSERT_VALUES,locRHS);CHKERRQ(ierr); /* get ghost values - exchange */
    ierr = DMGlobalToLocalEnd(dm,globalout,INSERT_VALUES,locRHS);CHKERRQ(ierr);
    ierr = VecGetArray(locRHS, &rhs_v);CHKERRQ(ierr);
    ierr = PetscMalloc((user->cEndInterior-cStart) * sizeof(PetscScalar), &uu);CHKERRQ(ierr);
    for (c = cStart; c < user->cEndInterior; ++c) {
      ...
      *pfDest = sten[0]*(*pfSrci);
      ...
      for (f = 0; f < numFaces; ++f) {
        ...
        else SETERRQ1(PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONGSTATE,"face with %d cells",nC);
        *pfDest += sten[1]*(*pfSrcj);
      }
    }
    /* copy back */
    for (c = cStart; c < user->cEndInterior; ++c) {
    ...
    ierr = VecRestoreArray(locRHS,&rhs_v);CHKERRQ(ierr);
    ierr = DMRestoreLocalVector(dm,&locRHS);CHKERRQ(ierr);
...
int main(int argc, char **argv)
{
  ...
  DM                dm, dmDist;
  PetscReal         ftime,dt;
  PetscInt          dim, overlap, nsteps, nrefine,i,ilev,N;
...
DMView(dm,PETSC_VIEWER_STDOUT_WORLD);
    ...
    if (vtkCellGeom) {
      ...
      ierr = OutputVTK(dm, "ex33-cellgeom.vtk", &viewer);CHKERRQ(ierr);
      ierr = VecView(user->cellgeom, viewer);CHKERRQ(ierr);
      ierr = PetscViewerDestroy(&viewer);CHKERRQ(ierr);
      ierr = CreatePartitionVec(dm, &dmCell, &partition);CHKERRQ(ierr);
      ierr = OutputVTK(dmCell, "ex33-partition.vtk", &viewer);CHKERRQ(ierr);
