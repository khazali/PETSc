...
static PetscErrorCode Monitor(TS ts,PetscInt step,PetscReal t,Vec U,void *ctx)
{
  ...
  while (user->next_output <= t && user->next_output <= tfinal) {
    ...
    ierr = PetscPrintf(PETSC_COMM_WORLD,"[%.1f] %D TS %.6f (dt = %.6f) X % 12.6e % 12.6e\n",
                       user->next_output,step,(double)t,(double)dt,(double)PetscRealPart(u[0]),
...
int main(int argc,char **argv)
{
  ...
  switch (mode) {
    case 2 :
      ...
      ierr = PetscPrintf(PETSC_COMM_WORLD,"%lg %lg\n%lg %lg\n",arr[0],arr[1],arr[2],arr[3]);CHKERRQ(ierr);
      break;
    case 1 : /* Compute the Hessian column by column */
      ...
      ierr     = PetscPrintf(PETSC_COMM_WORLD,"%lg\n%lg\n",arr[0],arr[1]);CHKERRQ(ierr);
      ierr     = VecCopy(x,user.U);CHKERRQ(ierr);
      ierr     = VecGetArray(user.Dir,&x_ptr);CHKERRQ(ierr);
      x_ptr[0] = 0.;
      x_ptr[1] = 1.;
...
/*TEST
    ...
    test:
      args:  -monitor 0 -ts_type cn -viewer_binary_skip_info -tao_view -mu 1000 -ts_dt 0.03125 -ts_trajectory_dirname ex20opt_icdir
    ...
    test:
      suffix: 4
      args: -implicitform 0 -ts_rhs_jacobian_test_mult_transpose -mat_shell_test_mult_transpose_view -tao_max_it 2 -ts_rhs_jacobian_test_mult -mat_shell_test_mult_view
TEST*/
