...
PetscErrorCode RHSFunction(TS ts,PetscReal ftime,Vec U,Vec F,void *ptr)
{
  ...
  */
  if (ys == 0) { /* south pole */
    for (i=xs; i<xs+xm; i++) {
      uarr[ys-1][i].u = -uarr[ys+1][i].u;
      uarr[ys-1][i].v = -uarr[ys+1][i].v;
      uarr[ys-1][i].h = uarr[ys][i].h;
    }
  }
  if (ys+ym == My) { /* north pole */
    for (i=xs; i<xs+xm; i++) {
      uarr[My][i].u = -uarr[My-2][i].u;
      uarr[My][i].v = -uarr[My-2][i].v;
...
PetscErrorCode RHSJacobian(TS ts,PetscReal t,Vec U,Mat A,Mat BB,void *ctx)
{
  ...
  Field          **uarr;
  Vec            localU;
  MatStencil     stencil[18],rowstencil;
  PetscScalar    entries[18];
  PetscErrorCode ierr;
  ...
  */
  ...
  /*
     Get pointers to vector data
  */
  ierr = DMDAVecGetArrayRead(da,localU,&uarr);CHKERRQ(ierr);
  ...
  */
  ...
  /*
     Place a solid wall at boundaries, velocities are reflected
  */
  if (ys == 0) { /* south pole */
    for (i=xs; i<xs+xm; i++) {
      uarr[ys-1][i].u = -uarr[ys+1][i].u;
  ...
  for (j=ys; j<ys+ym; j++) {
    ...
    if (j==0) stencil[6].j = j+1;
    if (j==My-1) stencil[0].j = j-1;
    ...
    for (i=xs; i<xs+xm; i++) {
      ...
      stencil[1].i = i-2; stencil[1].c = 0; entries[1] = alpha/2./a*PetscTanReal(lat)*vwp; /* uwp */
      ...
      stencil[5].i = i+2; stencil[5].c = 0; entries[5] = alpha/2./a*PetscTanReal(lat)*vep; /* uep */
      stencil[6].i = i; stencil[6].c = 0; entries[6] = -entries[0]; /* us */
      stencil[7].i = i-2; stencil[7].c = 1; entries[7] = alpha/2.*(fc+uwp/a*PetscTanReal(lat)); /* vwp */
      stencil[8].i = i; stencil[8].c = 1; entries[8] = -1./(2.*a*dlat)*(un-us)+(1.-alpha)*(fc+uc/a*PetscTanReal(lat)); /* vc */
      stencil[9].i = i+2; stencil[9].c = 1; entries[9] = alpha/2.*(fc+uep/a*PetscTanReal(lat)); /* vep */
      stencil[10].i = i-2; stencil[10].c = 2; entries[10] =  1./(2.*a*dlat)*2.*g/(sw->p*PetscCosReal(lat));/* hwp */
      stencil[11].i = i+2; stencil[11].c = 2; entries[11] = -entries[10]; /* hep */
  ...
  for (j=ys; j<ys+ym; j++) {
    ...
    for (i=xs; i<xs+xm; i++) {
      ...
      stencil[4].i = i-1; stencil[4].c = 1; entries[4] = 1./(2.*a*dlat)*uc/PetscCosReal(lat); /* vw */
      ...
      stencil[8].i = i; stencil[8].c = 2; entries[8] = -1./(2.*a*dlat)*2.*g/sw->q; /* hnq */
  ...
  for (j=ys; j<ys+ym; j++) {
    ...
    rowstencil.j  = j;
    for (i=xs; i<xs+xm; i++) {
      PetscReal uc,uep,uwp,uepnq,uwpnq,uepsq,uwpsq,vc,vnq,vsq,vepnq,vwpnq,vepsq,vwpsq,hc,he,hw;
      uc    = uarr[j][i].u;
      uep   = uarr[j][i+ph].u;
      uwp   = uarr[j][i-ph].u;
      uepnq = uarr[j+qh][i+ph].u;
      ...
      stencil[0].i = i-2; stencil[0].c = 0; entries[0] = 1./(2.*a*dlat)*2.*hc/PetscCosReal(lat)*alpha/2./sw->p; /* uwpnq */
      stencil[1].i = i+2; stencil[1].c = 0; entries[1] = -entries[0]; /* uepnq */
      stencil[2].i = i-2; stencil[2].c = 0; entries[2] = 1./(2.*a*dlat)*2.*hc/PetscCosReal(lat)*(1.-alpha)/sw->p; /* uwp */
      stencil[3].i = i; stencil[3].c = 0; entries[3] =  -1./(2.*a*dlat)/PetscCosReal(lat)*(he-hw); /* uc */
      stencil[4].i = i+2; stencil[4].c = 0; entries[4] = -entries[2]; /* uep */
      stencil[5].i = i-2; stencil[5].c = 0; entries[5] = entries[0]; /* uwpsq */
      stencil[6].i = i+2; stencil[6].c = 0; entries[6] = -entries[0]; /* uepsq */
      stencil[7].i = i-2; stencil[7].c = 1; entries[7] = -1./(2.*a*dlat)*2.*hc/PetscCosReal(lat)*alpha/2.*PetscCosReal(lat+qh*dlat)/sw->q; /* vwpnq */
      stencil[8].i = i; stencil[8].c = 1; entries[8] = -1./(2.*a*dlat)*2.*hc/PetscCosReal(lat)*(1.-alpha)*PetscCosReal(lat+qh*dlat)/sw->q; /* vnq */
      stencil[9].i = i+2; stencil[9].c = 1; entries[9] = entries[7]; /* vepnq */
      stencil[10].i = i-2; stencil[10].c = 1; entries[10] = 1./(2.*a*dlat)*2.*hc/PetscCosReal(lat)*alpha/2.*PetscCosReal(lat-qh*dlat)/sw->q; /* vwpsq */
      stencil[11].i = i; stencil[11].c = 1; entries[11] = 1./(2.*a*dlat)*2.*hc/PetscCosReal(lat)*(1.-alpha)*PetscCosReal(lat-qh*dlat)/sw->q; /* vsq */
      stencil[12].i = i+2; stencil[12].c = 1; entries[12] = entries[10]; /* vepsq */
      stencil[13].i = i; stencil[13].c = 2; entries[13] = -1./(2.*a*dlat)*vc; /* hn */
      stencil[14].i = i-1; stencil[14].c = 2; entries[14] = 1./(2.*a*dlat)*uc/PetscCosReal(lat); /* hw */
      stencil[15].i = i; stencil[15].c = 2; entries[15] = -1./(2.*a*dlat)*(2./PetscCosReal(lat)*((1.-alpha)*(uep-uwp)+alpha/2.*(uepnq-uwpnq+uepsq-uwpsq))/sw->p + 2./PetscCosReal(lat)*((1.-alpha)*(vnq*PetscCosReal(lat+qh*dlat)-vsq*PetscCosReal(lat-qh*dlat))+alpha/2.*(vepnq*PetscCosReal(lat+qh*dlat)-vepsq*PetscCosReal(lat-qh*dlat)+vwpnq*PetscCosReal(lat+qh*dlat)-vwpsq*PetscCosReal(lat-qh*dlat)))/sw->q); /* hc */
      stencil[16].i = i+1; stencil[16].c = 2; entries[16] = -entries[14]; /* he */
      stencil[17].i = i; stencil[17].c = 2; entries[17] = -entries[13]; /* hs */
      ...
      rowstencil.i = i;
      ierr = MatSetValuesStencil(A,1,&rowstencil,18,stencil,entries,ADD_VALUES);CHKERRQ(ierr);
    }
  }
  ...
  /*
     Restore vectors
  */
  ierr = DMDAVecRestoreArrayRead(da,localU,&uarr);CHKERRQ(ierr);
  ierr = DMRestoreLocalVector(da,&localU);CHKERRQ(ierr);
...
int main(int argc,char **argv)
{
  ...
  ierr = DMDACreate2d(PETSC_COMM_WORLD,DM_BOUNDARY_PERIODIC,DM_BOUNDARY_GHOSTED,DMDA_STENCIL_BOX,150,75,PETSC_DECIDE,PETSC_DECIDE,3,sw.p/2,NULL,NULL,&da);CHKERRQ(ierr);
