...
static PetscErrorCode MyMult(Mat A_shell,Vec X,Vec Y)
{
  ...
  PetscFunctionBeginUser;
  ierr = MatShellGetContext(A_shell,(void**)&mctx);CHKERRQ(ierr);
  ierr = RHSJacobian(mctx->ts,mctx->time,mctx->X,mctx->uctx->Jac,mctx->uctx->Jac,mctx->uctx);CHKERRQ(ierr);
...
int main(int argc,char **argv)
{
  ...
  ierr = MatCreateShell(PETSC_COMM_WORLD,2,2,2,2,&mctx,&A);CHKERRQ(ierr);
  ...
  ierr = MatCreateVecs(user.Jac,&x,NULL);CHKERRQ(ierr);
  ierr = VecDuplicate(x,&mctx.X);CHKERRQ(ierr);
  ...
  */
  ...
  ierr = TSSetRHSJacobian(ts,A,A,FormRHSJacobian,&user);CHKERRQ(ierr);
