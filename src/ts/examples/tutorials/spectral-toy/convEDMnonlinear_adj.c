...
extern PetscErrorCode RHSFunctionAdvectiongllDM(TS,PetscReal,Vec,Vec,void*);
extern PetscErrorCode RHSJacobianAdvectiongllDM(TS,PetscReal,Vec,Mat, Mat,void*);
...
int main(int argc,char **argv)
{
  ...
  appctx.param.Tadj =appctx.param.Tend+0.2;
  ...
  */ 
  ...
  ierr = VecView(appctx.dat.obj,viewfile);CHKERRQ(ierr);
   ...
   //exit(1);
  ierr = TSSetRHSFunction(appctx.ts,NULL,RHSFunctionAdvectiongllDM,&appctx);CHKERRQ(ierr);
  //ierr = TSSetRHSFunction(appctx.ts,NULL,TSComputeRHSFunctionLinear,&appctx);CHKERRQ(ierr);
  ierr = TSSetRHSJacobian(appctx.ts,appctx.SEMop.grad,appctx.SEMop.grad,RHSJacobianAdvectiongllDM,&appctx);CHKERRQ(ierr);
...
PetscErrorCode InitialConditions(Vec u,AppCtx *appctx)
{
  ...
  */
   ...
   */
  for (i=0; i<lenglob; i++) {
      s_localptr[i]=PetscExpScalar(-(xg_localptr[i]-2.0)*(xg_localptr[i]-2.0));
      }
...
PetscErrorCode Objective(PetscReal t,Vec obj,AppCtx *appctx)
{
  ...
  */
  for (i=0; i<lenglob; i++) {
      s_localptr[i]=PetscExpScalar(-(xg_localptr[i]-2.2)*(xg_localptr[i]-2.2));
        //PetscSinScalar(2.0*PETSC_PI*(xg_localptr[i]-0.2));
      } 
...
PetscErrorCode RHSFunctionAdvectiongllDM(TS ts,PetscReal t,Vec globalin,Vec globalout,void *ctx)
{
  ...
  //ierr = TSGetRHSJacobian(ts,&appctx->SEMop.grad,NULL,NULL,&ctx);CHKERRQ(ierr);
  ierr = VecDuplicate(appctx->dat.obj,&temp);CHKERRQ(ierr);
  //ierr = RHSMatrixAdvectiongllDM(ts,t,globalin,&appctx->SEMop.grad,NULL,ctx);CHKERRQ(ierr);

  // ierr = MatView(A,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr); 
  ierr = MatMult(appctx->SEMop.grad,globalin,temp);CHKERRQ(ierr);
  ierr = VecPointwiseMult(temp,globalin,globalout);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}

#undef __FUNCT__
#define __FUNCT__ "RHSJacobianAdvectiongllDM"
PetscErrorCode RHSJacobianAdvectiongllDM(TS ts,PetscReal t,Vec globalin,Mat A, Mat B,void *ctx)
{
  PetscErrorCode ierr;
  AppCtx        *appctx = (AppCtx*)ctx;  
 
  PetscFunctionBeginUser;
  MatCopy(A,appctx->SEMop.grad,SAME_NONZERO_PATTERN);
  MatDiagonalScale(A,appctx->dat.curr_sol,NULL);
