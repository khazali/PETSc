...
*/
#undef __FUNCT__
#define __FUNCT__ "ComputeGLLCoordinates"
PetscErrorCode ComputeGLLCoordinates(DM da,PetscGLLIP *gll)
{
  PetscErrorCode ierr;
  ...
  xs   = xs/(n-1);
  xn   = xn/(n-1);
  ierr = DMDASetUniformCoordinates(da,-1.,1.,0.,0.,0.,0.);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "ComputeSolution"
PetscErrorCode ComputeSolution(DM da,PetscGLLIP *gll,Vec u)
{
  PetscErrorCode ierr;
  PetscInt       j,xs,xn;
  PetscScalar    *uu,*xx;
...
*/
PetscErrorCode ComputeRhs(DM da,PetscGLLIP *gll,Vec b)
{
  PetscErrorCode ierr;
...
int main(int argc,char **args)
{
  ...
  for (n=4; n<N; n+=2) {
    ...
    */
    ...
    ierr = ComputeGLLCoordinates(da,&gll);CHKERRQ(ierr);

    /*
       Creates the element stiffness matrix for the given gll
    */
    ierr = PetscGLLIPElementStiffnessCreate(&gll,&A);CHKERRQ(ierr);
    ...
    */
    ...
    ierr = PetscGLLIPIntegrate(&gll,f,&norm);CHKERRQ(ierr);
    ...
    ierr = PetscGLLIPElementStiffnessDestroy(&gll,&A);CHKERRQ(ierr);
