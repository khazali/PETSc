...
The paper is organized as follows: the time dependent inverse
problem is stated in an operator fashion, which allows for any
operator such as diffusion, convection, or Burger's equation. Then the
main aspects of a spectral element method are presented together with
a discussion on implementation choices. We then present a variety of adjoint approaches, which are followed by A set of canonical test
problems, followed by an error analysis
discussion. Results of the PETSc/TAO implementation are included,
...
where $ P[ u ]$ is a stand-in operator for derivatives
...
Neumann. This notation does not exclude periodic boundary
conditions. For simplicity, in most of the presentation we assume only homogeneous Dirichlet and/or periodic boundary conditions; i.e.  $u|_{\partial \Omega}(t)= 0.$
...
u_t&=&{P}[u], \quad \text{with}\ u|_{t=0}=u_0\ , \text{and} \  B[u|_{\partial \Omega}(t)]= u_b(t)\ ,
\label{optimizationproblem}
\end{eqnarray}
where the partial differential equation is given by $ P$ and can be either advection-difussion, pure diffusion or possibly nonlinear combinations.

An important special case is the simple data assimilation problems for
which we seek the initial condition $u_0$ that leads at time $T$ to a
solution $u$ which matches a reference solution $u_d$. In practice $
u_d$ is not known over the entire domain $\Omega$, but in a subset
region $\Omega_s\subset \Omega$ where sensors have been
placed. However, for simplicity we construct a continuous function
$u_d$ to avoid regularization issues, and consider the sensor values
$u_s$ to be a restriction of the destination vector $u_d$,
i.e. $u_s=u_d|_{\Omega_s}$.  A cost functional which minimizes the
difference between $| u- u_d|$ while preserving convexity is
\begin{equation}
...
%Consider a time dependent partial differential equation with constant coefficients, given by the spatial operator $  P( u)$ and initial conditions $ g(\cdot)$ as
%\begin{eqnarray}
% \frac{\partial u}{\partial t} &=&   P( u) \ ,  x \in \Omega \\ \nonumber
%   u|_{\partial \Omega}&=&0, \\ 
%   u(0,t)&=& g( x) \ . \nonumber \label{eq:generic}
%\end{eqnarray} 

To transition from the strong form of the equation, (\ref{optimizationproblem}), to the weak form we define the spaces
$$
...
We define the space
\begin{eqnarray*}
K  =   K (\Omega,T) & = & \lbrace \bigg(\int_0^T \int_{\Omega} |u|^2 \d \Omega \d t \bigg) < \infty | \\
                  &   &  \bigg(\int_0^T \int_{\Omega} |u_t|^2 \d \Omega \d t \bigg) < \infty | \\
                  &   &  \bigg(\int_0^T \int_{\Omega} |\frac{\partial u}{\partial  x}|^2 \d \Omega \dt \bigg) < \infty | \quad  u|_{\partial\Omega}(t) = 0 \rbrace.
\end{eqnarray*}
...
that is
\[
\int_0^T \int_{\Omega} ( u_t- P( u))  v \d \Omega \d t = 0
\]
for all $ v$ in $K$.

Example: Let $ P( u) = \nu \Delta  u +  a \cdot \nabla  u$, where $\nu$  represents the viscosity, and $ a$ is the advection velocity. Apply integration by parts to remove any second derivatives from the equation to obtain
\begin{equation}
...
Based on the weak form given by Equation~\ref{eq:weakcd} several
discretizations are suitable, the finite element method, spectral
elements, continuous or discontinuous Galerkin, etc.  The spectral
element method is a subclass of Galerkin methods, or weighted residual
methods.  The idea behind Galerkin methods is to minimize the error of
the numerical computation in the energy norm over a chosen space of
polynomials, or, equivalently require the error to be orthogonal to
the subspace defined by the spectral elements.
...
In the case of a spectral element method the domain $\Omega=\cup_{e=1,M} \Omega_e $, is decomposed into $M$ nonoverlapping 
subdomains $\Omega_e$, termed elements, over which the data will be represented by orthogonal polynomials.

Let us now define $X=H^1_0(\Omega)^d$. The space of polynomials of order $N$ defined over an element $\Omega_e, \ e=1,\ldots, E$ is
$$
\mathbb P_{N,E}=\lbrace  \phi| \phi \in L^2(\Omega); \quad \phi|_{\Omega_e} \text{polynomial of degree} \leq N\rbrace,
$$
Subsequently $X_N=X\cap  P_{N,E}^d$, where $d$ is the dimension of the problem.

In the polynomial space $X_N$ we expand the numerical solution 
\begin{equation}
u( x)=\sum_{i=1}^N u_{i}\phi_i( x)\ .
\label{eq:ansatz}
\end{equation}

The discrete point distribution $ x$ can be either a Chebyshev grid, or a Legendre grid, in either case the polynomials are orthogonal $\int_{\Omega} \phi_i, \phi_j \d \Omega =\delta_{ij}$. 
...
\begin{equation}
\frac{\partial}{\partial t}\int \sum_{i=1}^N u_{i}\phi_i(x) \sum_{j=1}^N v_{j}\phi_j(x) \d \Omega + \int\sum_{i=1}^N u_{i}\phi'_i(x) \sum_{j=1}^N v_{j}\phi'_j(x) \d \Omega  +\int\sum_{i=1}^N u_{i}\phi_i(x) \sum_{j=1}^N v_{j}\phi'_j(x) \d \Omega =0 \ .
\end{equation}
With suitable reordering this results in
\begin{equation}
\frac{\partial}{\partial t}\sum_{i,j=1}^N u_{i}v_{j}\underbrace{\int_{\Omega} \phi_i(x)\phi_j(x) \d \Omega}_{M_{ij}} + \sum_{i,j=1}^N u_{i} v_{j}\underbrace{\int_{\Omega} \phi'_i(x) \phi'_j(x) \d \Omega}_{K_{ij}} +\sum_{i,j=1}^N u_{i} v_{j}\underbrace{\int_{\Omega}\phi'_i(x) \phi_j(x) \d \Omega}_{D_{ij}}  =0
\end{equation}
where $M_{ij}$ is the mass matrix and $K_{ij}$ is the stiffness matrix. Due to the orthogonality properties of the polynomials the mass matrix is diagonal. 
In algebraic form we can write the equations as
\begin{equation}
...
If we presume  $\Omega_e = [a, \ b]$ and define the reference element $\hat{\Omega} = [-1, \ 1]$.
then for $x\in \Omega_e$ and $r \in \hat{\Omega}$ the mapping from each element to the reference element is
$$x= a+ \frac{b-a}{r+1}\ .$$
This mapping introduces a scaling factor for each element, given by the Jacobian of the coordinate transformation.
...
\begin{itemize}
\item replace the advection velocity $ u$ by a constant velocity field $c$ that is divergence free
...
\int_{\Omega}\frac{\partial u}{\partial t} \cdot  v\d \Omega &=&\frac{\partial}{\partial t} \int_{\Omega}u \cdot  v\d \Omega\\
\int_{\Omega} \nabla\cdot \nabla u\cdot v\d \Omega &=&\int_{\Omega}  \nabla u\cdot \nabla v\d \Omega +\int_{\partial\Omega} v \nabla u \cdot  n\d\Omega
\end{eqnarray}
...
\int_{\Omega}\frac{\partial u}{\partial t} \cdot  v\d \Omega &=&\frac{\partial}{\partial t} \sum_i^N\sum_j^N v_i\bigg(\int_{\Omega}  \phi_i( x) \cdot \phi_j( x)\d \Omega u_j\bigg) \\
\int_{\Omega}(c \cdot \nabla u)\cdot  v\d \Omega\ &=&\sum_i^N\sum_j^N v_i\bigg (\int_{\Omega}(c \cdot \phi_i( x)\cdot \nabla\phi_j( x)  \d \Omega\bigg) u_j\ \\
\int_{\Omega} (\nabla\cdot \nabla  u)\cdot  v\d \Omega &=&\sum_{i=1}^N \sum_{j=1}^N v_{i}(\int_{\Omega}\nabla\phi_i( x) \cdot \nabla \phi_j( x)\d \Omega)u_j \\
\end{eqnarray}
...
$\int_{-1}^1\phi( x)\d x=\sum_k \rho_k\phi(x_k)$
...
\begin{equation}
A_{ij}=\int_{\Omega}\phi'_i( x) \phi'_j( x)\d \Omega=\frac{L}{2}\int_{-1}^1\phi'_i(r) \phi'_j(r)\d r=\frac{L}{2}\sum_k \rho_k\phi'_i(r_k) \phi'_j(r_k)
\end{equation}
and the convection operator
\begin{equation}
C_{ij}=\int_{\Omega}c\phi_i( x) \phi'_j( x)\d \Omega=\frac{L}{2}\int_{-1}^1c(r)\phi_i(r) \phi'_j(r)\d r=\frac{L}{2}\sum_k c(r_k)\rho_k\phi_i(r_k) \phi'_j(r_k)
\end{equation}
...
%\begin{equation}
%   u( x)|_{\Omega_e} = \sum_{i,j,k} u^e_{ijk}
...
Several approaches are available when dealing with PDE-constrained
optimization problems; one family can be cast in a Lagrange-multiplier
framework.
Variational calculus allows us to obtain a direct derivation of the adjoint equations, which are weakly decoupled from the forward PDE. By weakly decoupled in this context we mean that the backward and forward problems can be regarded as two independent PDEs where the backward problem is tied to the forward via initial conditions, boundary conditions, and the advection velocity in the case of nonlinear problems.

We summarize in Table~\ref{sum} the approaches available for PDE constrained optimization.

\begin{table}
\centering
\begin{tabular}{|r|c|c|}
  \hline
   & Parallel in time & Adjoint looping \\
  \hline\hline
...
$${L}[ u,  v]=\int_{\Omega}( u[T]- u_d)^2 \ \d \Omega +\int_0^T\int_{\Omega} P[ u]  v \ \d \Omega \d t $$
...
Focusing on the term $ P[u]$ and presuming it stands for a diffusion operator we rewrite
\begin{eqnarray}
...
\begin{eqnarray}
{L}[ u, u_0,v]&=&\int_{\Omega}( u[T]- u_d)^2 + \int_{\Omega} u[T] v[T] - u[0] v[0]\ \d \Omega+\int_0^T\int_{\Omega} \overline{P}[ v]  u \ \d \Omega \d t + \\ \nonumber
 && \int_0^T\int_{\Gamma}(\nabla  u\cdot  v-  u\cdot \nabla v) n\ \d \Gamma \d t
\end{eqnarray}

Here we denote by $\overline{ P}$ the reverse mode which is reversed in time, i.e. $\overline{ P}[u(x,t)]=-\overline{ P}[u(x,-t)]$ and determines the backward heat equation in this case. 

In order to take a derivative of a functional we need to make use of the Riesz representation theorem, which states that for a functional $ J$, the derivative is $d  J[u]/dv=(u,v)(=\int_{\Omega} u\cdot v \ d \Omega)$. Considering now each partial derivative individually we have for the Lagrangian
\begin{eqnarray}
\nabla_u  L&=&- \frac{\partial v}{\partial t}  +\nu\Delta  v \\ \nonumber
\nabla_{u_0}  L&=&- v[0]\\ \nonumber
\nabla_{u_T}  L&=&2( u[T]- u_d)+ v[T]\\ \nonumber
\label{eq:bcs_ics}
\end{eqnarray}
...
\end{align}
which is identical with Equation~\ref{eq:generic}, with the difference that now we need an explicit definition of the spatial operator at every time step so we took $f(u,p,t)= P[u,p](t)$, previously it was sufficient to consider the dependency of $ P$ on $u$. Note that in fact the optimization parameter remains $\param=u_0$, however we shall write $p$ to avoid confusion with the discrete timestep $u_0$. 
...
$${L}[ u,  u_0]=\int_{\Omega}( u[T]- u_d)^2 \ \d \Omega +\int_0^T\int_{\Omega} P[ u]  v \ \d \Omega \d t + \int_0^T\int_{\Gamma}  v_b  ( u-  u_b) \ \d \Gamma \d t$$
...
Grouping now the time dependent terms we have
\begin{eqnarray}
{L}[ u,  u_0,  v]&=&\int_{\Omega}( u[T]- u_d)^2 + u[T] v[T]\ \d \Omega -\int_{\Omega} u[0] v[0]\ \d \Omega+\int_0^T\int_{\Omega} \overline{P}[ v]  u \ \d \Omega \d t + \\ \nonumber
...
\frac{\partial L}{\partial  u_T}&=&2( u[T]- u_d)+ v[T]\\
