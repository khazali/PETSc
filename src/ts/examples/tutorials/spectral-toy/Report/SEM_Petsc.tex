...
A PDE constrained optimization problem can be stated in two different ways according to the problem at hand: either by fully discretizing the entire KKT (Karush-Kuhn-Tucker) system, as is customary in the optimization community \cite{Haber_2007}, or by decoupling the direct problem and its adjoint and solving them via a forward/backward timestepping loop, with appropriate initial and boundary conditions depending on the objective function.
...
where $\mathcal P[ u ]$ is a stand-in operator for derivatives controlling the spatial behaviour of the solution $u$. The boundary condition $u_b$ is also provided by an operator $B[u]$ which can be the identity for Dirichlet boundary conditions or a derivative for Neumann, while $u_b(t)$ denotes whether the boundaries are homogeneously or inhomogeneously satisfied, or whether they are time dependent. We draw the attention that this notation does not exclude periodic boundary conditions, which can be enforced by a quasi-identity operator $B[\cdot]$ such that $B[u_0]=u_N$ and $B[u_N]=u_0$, while $B[u_i]=u_i$ for $i=1,\ldots,N-1$ for all $t\in [0,\ T]$.
...
\begin{equation}J[u_0]=\int_0^T\int_{\Omega}(u- u_d)^2 \ \d \Omega \,.  \label{eq:Cost_general}
\end{equation}

The optimization problem we seek to solve is
\begin{eqnarray}
\min_{u_0} &&J[u_0]\\ \nonumber
u_t&=&\mathcal{P}[u], \quad \text{with}\ u|_{t=0}=u_0\ ,
\end{eqnarray}
where the partial differential equation is given by $\mathcal P$ and can be either advection-difussion, pure diffusion or other combinations thereof.
...
$(\mathbf u_t-\mathcal P(\vect u),\mathbf v)=0$ for all $\mathbf v$ in a given test space, under the inner product defined as
$$(\mathbf u,\mathbf v)=\int_{\Omega} \mathbf u\ \mathbf v \ \d \Omega$$

To take a concrete example let us set $\mathcal P(\mathbf u) = \nu \Delta \mathbf u + \mathbf a  \nabla \mathbf u$, where $\nu$ is a parameter which represents the viscosity, and $\mathbf a$ is the advection velocity. To derive the weak form we multiply both sides of the partial differential equation (Eq.~\ref{eq:generic}) by a test function $\mathbf v$ and subsequently apply integration by parts
\begin{equation}
\int_{\Omega}\frac{\partial\mathbf u}{\partial t} \mathbf v \ \d \Omega + \int_{\Omega} \nu\nabla \mathbf u\ \nabla \mathbf v\ \d \Omega +\int_{\Omega} \mathbf a \cdot \mathbf v\ \nabla \mathbf u\ \d \Omega-
 \int_{\Gamma} \nabla \mathbf u \ \mathbf v\  \d \Gamma =0\label{eq:weakcd}
\end{equation}
...
Based on the weak form given by Eq.~\ref{eq:weakcd} several discretizations are suitable, Finite Element Method, Spectral Element, Continuous or Discontinuous Galerkin, etc.
...
In the following we illustrate the spectral element discretization for the convection-diffusion equation Eq.~\ref{eq:weakcd}, restricting ourselves to the one dimensional case
\begin{equation}
...
To proceed with the numerical discretization we plug Eq.~\ref{eq:ansatz} into Eq.~\ref{eq:weakcd} to obtain
\begin{equation}
...
Two main approaches are available when dealing with a PDE-constrained optimization problem and both can be cast in a Lagrange-Multipliers framework. Although the Lagrange-Multipliers framework is not the sole path to solution for these types of optimization problems we choose it here for consistency, furthermore it offers a suitable way to illustrate the connections between continuous and discrete adjoints.

Variational calculus allows us to obtain a direct derivation of the adjoint equations, weakly decoupled from the forward PDE, which stands as the constrain for the optimization. By weakly decoupled in this context we mean that the backward and forward problems can be regarded as two independent PDEs however the backward problem is tied to the forward via initial conditions, boundary conditions as well as advective velocity in the case of nonlinear problems.

In the examples we consider here the objective function will be restricted to the final time horizon which implies that in Eq.\ref{eq:Cost_general} we have $\mathcal{J}[u_0]=\int_{\Omega}( u[T]- u_d)^2 \ \d \Omega$.


\subsection{Variational calculus, Continous adjoint\label{sec:adj:continuous}} 

To derive the gradient used in the backward step we set up the Lagrangian
...
\begin{eqnarray}
\mathcal{L}[ u, u_0,  v]&=&\int_{\Omega}( u[T]- u_d)^2 + u[T] v[T]\ \d \Omega -\int_{\Omega} u[0] v[0]\ \d \Omega+\int_0^T\int_{\Omega} \overline{P}[ v]  u \ \d \Omega \d t + \\ \nonumber
 && \int_0^T\int_{\Gamma}(\nabla  u\cdot  v-  u\cdot \nabla v) n\ \d \Gamma \d t
\end{eqnarray}

Here we denoted by $\overline{\mathcal P}$ the reverse mode which coincides with the forward but reversed in time, i.e. $\overline{\mathcal P}[u(x,t)]=-\overline{\mathcal P}[u(x,-t)]$ and determines the backward heat equation in this case. Summing up all the partial derivatives of the Lagrangian we have
\begin{eqnarray}
...
Let us assume we have a time-dependent problem such as Eq.~\ref{eq:1dcd} where again we omit the convective term, since it far less problematic. In semi-discrete form we write
%
\begin{align}
  \label{eq:semi:discrete:form}
\frac{\partial u}{\partial t} = f(u,p,t)\,, 
\end{align}
which is identical with Eq.~\ref{eq:generic}, with the difference that now we need an explicit definition of the spatial operator at every time step so we took $f(u,p,t)=\mathcal P[u,p](t)$, previously it was sufficient to consider the dependency of $\mathcal P$ on $u$. Note that in fact the optimization parameter remains $\param=u_0$, however we shall write $p$ to avoid confusion with the discrete timestep $u_0$. 
...
Eq.~\ref{eq:Cost_general} where for brevity we set the notation $g(u)=\int_{\Omega} (u-u_d)^2 \d \Omega$. We set for instance a simple Riemann summation
...
\end{align}
%
The Lagrangian function for Eq.~\ref{eq:discrete:optim} is
%
\begin{align}
  \nonumber
...
The gradient based optimization can be performed using adjoint information, or as a coupled KKT system \cite{Haber_2007}. Here we showcase the adjoint based approach, which as illustrated in Fig.~\ref{fig:algorithm} consists of two solvers, one forward and one backward in time.
...
In the current framework we consider a set of problems in one dimension, diffusion, convection-diffusion, and diffusion with a discretely discontinous objective function. All spatial operators are discretized using the spectral element method, and the time integration is based on the TS capability of PETSc.

\subsection{Advection-Diffusion}
We consider Eq.~\ref{eq:1dcd} and seek $u_0$ which minimizes $\int (u|_T-u_d)^2\d \Omega$. 
...
In Fig.\ref{fig:ic_obj} we illustrate both the initial condition and desired solution we seek to attain at time horizon $T$. The cost function decay versus the number of iterations required for finding the initial condition $u_0$ is illustrated in Fig.~\ref{fig:decay_cd}. For an admissible error in the solution of $10^{-8}$ we obtain the final optimal in Fig.\ref{fig:opt_cd}. The optimization algorithm used here is quasi-Newton algorithm under TAO, specifically Limited Memory Variable Metric (\texttt{LMVM}), and the timestepper is an explicit Runge-Kutta of 3rd order accuracy.


\subsection{Diffussion}

A diffusion problem with periodic boundary conditions and viscosity parameter $\nu=0.001$ is considered over the domain $[0,1]$. The domain is discretized in 5 elements each of polynomial order $N=8$ and the time horizon of the forward problem is $T=1$. We distinguish two cases one where the objective function is smooth, and the desired end state $u_d$ is discretized over the entire domain, and a second case where $u_d$ is not available at every grid in the domain, but a scattered selection of points. The smooth continous case is illustrated in Fig.~\ref{fig:ic_obj_heat}, and Fig.~\ref{fig:opt_heat}. We note a difference between the optimal and the objective, which is only natural given that the initial optimal condition has to decay for a time $T$ to reach $u_d$.

The case where the objective is only a discrete set of points $N_s=2 N/3$, a two thirds fraction of the total spatial discretization $N$, is depicted in Fig.~\ref{fig:opt_heatass}. This result is decieving since it resembles that the optimal initial condition is almost superposed to the destination state, however this as we know by comparison with the continous $u_d$ is not consistent with the travel time to destination $T$.    
...
\begin{figure}[!ht]
\centering
\subfloat[Initial condition(black), desired profile(blue)-only a set of discrete points, optimal solution(red)]
{\includegraphics[width=0.47\textwidth]{Heatdataass.eps}
\label{fig:ic_obj_heatass}}
\quad
\subfloat[Decay of the cost function for a pointwise discrete objective function, convergence plateaus early on.]
{\includegraphics[width=0.47\textwidth]{Dataass_decay.eps}
...
This operation is performed per element and for a given signal such as the one in Fig.~\ref{fig:err_tot} we obtain error bounds per element that are consistent with the signal fluctuations as illustrated in Fig.~\ref{fig:err_tot}. As a general procedure we perform a least square fit in spectral space to identify the value of the parameter $\sigma$. To certify the consistency of the fit with the spectral decay we show in Fig.~\ref{fig:err_spec} the estimated error decay versus the actual spectral error.
