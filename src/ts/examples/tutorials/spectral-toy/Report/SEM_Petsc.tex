...
For the weak form we seek all functions with the property that residual is orthogonal to the set o all test functions.
...
In alegraic form we can rewrite
...
\begin{eqnarray}
 \frac{\partial\mathbf u}{\partial t} - \nu\Delta \mathbf u +\mathbf f(\mathbf x)&= &0, \ \mathbf x \in \Omega \\ \nonumber
  \mathbf u|_{\partial \Omega}&=&\mathbf u_b, \\ \nonumber
  \mathbf u(\mathbf x,0)&=&\mathbf u_0(\mathbf x)\label{eq:heateq}
\end{eqnarray} 

As a generic notation we use for the PDE
$$\mathcal P [\mathbf u]=0$$. 

Presume we seek to find what forcing $\mathbf f$ acting on the entire domain $\Omega$ yields a solution $\mathbf u$ which matches a reference solution $\mathbf u_d$ at time $T$. Typically $\mathbf u_d$  is not known over the entire domain $\Omega$ in practice, but in a subset region $\Omega_s$ where sensors have been placed. However for simplicity here we construct a continous function $\mathbf u_d$ to avoid regularization issues.

The approach is to construct a cost function which minimizes the difference between $|\mathbf u-\mathbf u_d|$.

$$J[\vect u]=\int_{\Omega}(\mathbf u-\mathbf u_d)^2 \ \d \Omega  $$

We seek $\vect u$ solution to Eq.~\ref{eq:heateq} such that $J[\vect u]$ is minimal.
\subsection{Variational calculus derivation}
...
To validate and track convergence we shall construct the solution via method of manufactured solutions.
So we consider $\mathbf u_0$ and compute a $\mathbf f_{\mathbf u_0}$ which yields the solution $\underline{\mathbf u}$.
By construction the inverse problem has the solution $\mathbf f_{\mathbf u_0}$
$$\mathcal P (\underline{\mathbf u}, \mathbf f_{\mathbf u_0})=0$$. 
Algortihm
\begin{itemize}
\item Choose $\mathbf u_0$ such that $\mathbf f_{\mathbf u_0}$ is non trivial
\item Store $\underline{\mathbf u}$ solution to $$\mathcal P (\underline{\mathbf u}, \mathbf f_{\mathbf u_0})=0$$. 
\item Initialize PDE with $\mathbf u_0$, but with a different $\mathbf f_{n}$, e.g. $\mathbf f_{init}=1$
\item Perform optimization problem (Forward/Dual) until  $\min_{f}J[f]=\int_0^T\int_{\Omega}\mathbf u-\underline{\mathbf u} \ \d x \d t \approx 0$
\item Addtionally track at each iteration $|\mathbf f_{n}-\mathbf f_{\mathbf u_0}|$
