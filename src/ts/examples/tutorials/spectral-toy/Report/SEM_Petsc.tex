...
\begin{eqnarray}
 \frac{\partial\mathbf u}{\partial t} - \nu\Delta \mathbf u +\mathbf f(\mathbf x)&= &0, \ \mathbf x \in \Omega \\ \nonumber
  \mathbf u|_{\partial \Omega}&=&0, \\ \nonumber
  \mathbf u(0,t)&=&\mathbf u_0(\mathbf x)\label{eq:heateq}
\end{eqnarray} 

As a generic notation we use for the PDE
$$\mathcal P (\mathbf u, \mathbf f)=0$$. 

Presume we seek to find what forcing $\mathbf f$ acting on the entire domain $\Omega$ yields a solution $\mathbf u$ which matches a reference solution $\underline{\mathbf u}$ at time $T$. Typically $\underline{\mathbf u}$  is not known over the entire domain $\Omega$ in practice, but in a subset region $\Omega_s$ where sensors have been placed. However for simplicity here we construct a continous function $\underline{\mathbf u}$ to avoid regularization issues.

The approach is to construct a cost function which minimizes the difference between $|\mathbf u-\underline{\mathbf u}|$.

$$J[f]=\int_0^T\int_{\Omega}(\mathbf u-\underline{\mathbf u})^2 \ \d x \d t $$

We seek $\mathbf f$ solution to Eq.~\ref{eq:heateq} such that $J[f]$ is minimal.
