...
where $\mathcal P[ u ]$ is a stand-in operator for derivatives controlling the spatial behaviour of the solution $u$. The boundary condition $u_b$ is also provided with an operator $B[u]$ which can be the identity for Dirichlet boundary conditions or a derivative for Neumann, while $u_b(t)$ denotes whether the boundaries are homogeneously or inhomogeneously satisfied, or whether they are time dependent. This notation does not exclude periodic boundary conditions, which can be enforced by a quasi-identity operator $B[\cdot]$ such that $B[u_0]=u_N$ and $B[u_N]=u_0$, while $B[u_i]=u_i$ for $i=1,\ldots,N-1$ for all $t\in [0,\ T]$.
...
and the space
$$
H^1_0(\Omega)=\lbrace f \in H^1(\Omega) |\quad \ f|_{\partial\Omega}=0 ,\ \rbrace
$$
...
$(\mathbf u_t-\mathcal P(\vect u),\mathbf v)=0$ for all $\mathbf v$ in a given test space, under the usual inner product defined as
$$(\mathbf u,\mathbf v)=\int_{\Omega} \mathbf u\ \mathbf v \ \d \Omega$$

To take a concrete example let us set $\mathcal P(\mathbf u) = \nu \Delta \mathbf u + \mathbf a  \nabla \mathbf u$, where $\nu$ is a parameter which represents the viscosity, and $\mathbf a$ is the advection velocity. To derive the weak form we multiply both sides of the partial differential equation (Equation~\ref{eq:generic}) by a test function $\mathbf v$ and apply integration by parts
\begin{equation}
