...
int main(int argc,char **argv)
{
  ...
  PetscMPIInt    size;
  PetscReal      x, *wrk_ptr1, *wrk_ptr2, *wrk_ptr3, wrk1;
  ...
  appctx.param.mu    = 0.01; 
 printf("mu %g\n", appctx.param.mu);
  appctx.param.steps =20000000;

  appctx.initial_dt=1e-4;
  appctx.param.Tend = 0.005; //appctx.param.steps*appctx.param.dt;  
  appctx.param.Tadj =appctx.param.Tend+2.0;
 ...
 for (i=xs; i<xs+xm; i++) {
  ...
  ierr = VecView(appctx.dat.obj,viewfile);CHKERRQ(ierr);
   ...
   ierr = MatDuplicate(appctx.SEMop.stiff,MAT_COPY_VALUES,&appctx.SEMop.jac);

   VecDuplicate(appctx.dat.ic,&appctx.dat.curr_sol);
   VecCopy(appctx.dat.ic,appctx.dat.curr_sol);
   ...
   //ierr = TSSetRHSJacobian(appctx.ts,appctx.SEMop.jac,appctx.SEMop.jac,RHSJacobianBurgersgllDM,&appctx);CHKERRQ(ierr);
  ...
  ierr = TaoSetTolerances(tao,1e-12,PETSC_DEFAULT,PETSC_DEFAULT);CHKERRQ(ierr);
  ...
  ierr = MatDestroy(&appctx.SEMop.jac);CHKERRQ(ierr);
...
PetscErrorCode InitialConditions(Vec u,AppCtx *appctx)
{
  ...
  */
   ...
   */
  for (i=0; i<lenglob; i++) {
       //s_localptr[i]=PetscExpScalar(-(xg_localptr[i]-10.0)*(xg_localptr[i]-10.0));
      s_localptr[i]=2.0*appctx->param.mu*PETSC_PI*PetscSinScalar(PETSC_PI*xg_localptr[i])/(2.0+PetscCosScalar(PETSC_PI*xg_localptr[i]));//+PetscExpScalar(-4.0*(xg_localptr[i]-2.0)*(xg_localptr[i]-2.0))*4.0*appctx->param.mu;
      }
...
PetscErrorCode Objective(PetscReal t,Vec obj,AppCtx *appctx)
{
  ...
  */
  for (i=0; i<lenglob; i++) {
      s_localptr[i]=2.0*appctx->param.mu*PETSC_PI*PetscSinScalar(PETSC_PI*xg_localptr[i])*PetscExpScalar(-PETSC_PI*PETSC_PI*t*appctx->param.mu)\
              /(2.0+PetscExpScalar(-PETSC_PI*PETSC_PI*t*appctx->param.mu)*PetscCosScalar(PETSC_PI*xg_localptr[i]));
      //PetscExpScalar(-(xg_localptr[i]-2.2)*(xg_localptr[i]-2.2));
      } 
...
PetscErrorCode RHSMatrixHeatgllDM(TS ts,PetscReal t,Vec X,Mat A,Mat BB,void *ctx)
{
  ...
  Mat            K,Q;
   ...
   MatScale(K,2.0/appctx->param.Le);
   MatDiagonalScale(K,appctx->SEMop.massinv,NULL);
  ...
  /*
   ...
   //MatCopy(A,AA,DIFFERENT_NONZERO_PATTERN);
     return 0;
}
...
PetscErrorCode FormFunctionGradient(Tao tao,Vec IC,PetscReal *f,Vec G,void *ctx)
{
  ...
  PetscErrorCode    ierr;
  Vec               temp, temp2,buah;
  PetscInt          its, i, lenglob;
  PetscReal         ff, gnorm, cnorm, xdiff,fr, *wrk1, *wrk2, *s_localptr; 
...
ierr = VecDuplicate(appctx->dat.curr_sol,&buah);CHKERRQ(ierr);
ierr = VecCopy(appctx->dat.curr_sol,buah);CHKERRQ(ierr);
  /*
  ...
  */
  ...
  ierr = DMDAVecGetArray(appctx->da,temp,&wrk1);CHKERRQ(ierr);
  ierr = DMDAVecGetArray(appctx->da,appctx->dat.curr_sol,&wrk2);CHKERRQ(ierr);
  //ierr = DMDAVecGetArray(appctx->da,appctx->dat.curr_sol,&s_localptr);CHKERRQ(ierr);

  lenglob  = appctx->param.E*(appctx->param.N-1)+1;
  
  for (i=0; i<lenglob; i++) {
      wrk1[i]=wrk1[i]-wrk2[i];
      }

  //ierr = DMDAVecRestoreArray(appctx->da,appctx->SEMop.grid,&xg_localptr);CHKERRQ(ierr);
  ierr = DMDAVecRestoreArray(appctx->da,temp,&wrk1);CHKERRQ(ierr);
 ...
 ierr = VecCopy(appctx->dat.curr_sol,appctx->dat.grad);CHKERRQ(ierr);
 // if(appctx->AdjTestFD){
    /*VecView(appctx->dat.curr_sol,PETSC_VIEWER_STDOUT_WORLD);*/
 //   ierr = TSSetCostGradients(appctx->ts,1,&appctx->dat.curr_sol,NULL);CHKERRQ(ierr);
  ...
  ierr = PetscObjectSetName((PetscObject)appctx->dat.obj,  "Obj");
  ierr = VecView(appctx->dat.obj,viewfile);CHKERRQ(ierr);
  ierr = PetscObjectSetName((PetscObject)buah,  "fwd");
  ierr = VecView(buah,viewfile);CHKERRQ(ierr);
...
Mat A;
  ...
  //ierr = MatMult(appctx->SEMop.stiff,globalin,temp);CHKERRQ(ierr);
  //ierr = VecAXPY(globalout,1.0,temp);CHKERRQ(ierr);     //laplace u - u grad u
...
PetscErrorCode RHSJacobianBurgersgllDM(TS ts,PetscReal t,Vec globalin,Mat A, Mat B,void *ctx)
{
 ...
 PetscViewer        viewfile;
  ...
  //MatCopy(appctx->SEMop.stiff,A,SAME_NONZERO_PATTERN);
  //MatScale(A, -1.0);
/*
