...
PetscErrorCode TSQuadrature_PostStep(TS ts)
{
...
PetscErrorCode TSSolveWithQuadrature_Private(TS ts, Vec X, Vec design, Vec direction, Vec quadvec, PetscReal *quadscalar)
{
  ...
  if (direction) { /* when the direction is present, the ts need to be a TLMTS */
    ...
    if (ts->funchead) SETERRQ(PetscObjectComm((PetscObject)ts),PETSC_ERR_PLIB,"Objective functions already linked to TLMTS");
    ts->funchead = model->funchead;
  }
  ...
  if (qeval_ctx->veval) {
    ...
    if (direction) { /* Hessian computations */
      ...
      if (model->HF[2][0] || model->HF[2][1] || model->HF[2][2]) has = PETSC_TRUE;
    } else {
  ...
  if (qeval_ctx->veval || veval_fixed) {
    if (!direction) {
      ...
      ierr = PetscObjectQuery((PetscObject)model,"_ts_hessian_foats",(PetscObject*)&q->adjts);CHKERRQ(ierr); /* XXX */
      if (!q->adjts) SETERRQ(PetscObjectComm((PetscObject)model),PETSC_ERR_PLIB,"Missing first-order adjoint");
