...
PetscErrorCode  TSAdaptSetType(TSAdapt adapt,TSAdaptType type)
{
  ...
  PetscFunctionBegin;
  ierr = PetscFunctionListFind(TSAdaptList,type,(void(**)(void))&r);CHKERRQ(ierr);
...
PetscErrorCode TSAdaptChoose(TSAdapt adapt,TS ts,PetscReal h,PetscInt *next_sc,PetscReal *next_h,PetscBool *accept)
{
  ...
  if (!(*next_h > 0.)) SETERRQ1(PetscObjectComm((PetscObject)adapt),PETSC_ERR_ARG_OUTOFRANGE,"Computed step size %G must be positive",*next_h);
