...
static PetscErrorCode TSStep_Multi(TS ts)
{
    ...
    PetscFunctionBegin;
    // ? Why does TSPreStep have to be called here, yet TSPostStep gets called in TSSolve?
...
static PetscErrorCode TSSetUp_Multi(TS ts)
{
    ...
    void           *ctx;
    DM             dm; //<-- get rid of once we add TSRHSSetPartitionFunction
   ...
   ierr = TSSetSolution(multi->tsFine,ts->vec_sol);CHKERRQ(ierr);
    ...
    ierr = DMTSGetRHSPartitionFunction(dm,MULTI,MULTI_SLOW,&rhsFuncSlow,&ctx);CHKERRQ(ierr); //<<< once we have a real package, this should be changed to MULTI_FLAVOR_SLOW
