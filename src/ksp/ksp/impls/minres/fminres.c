...
#undef __FUNCT__
#define __FUNCT__ "KSPSetUp_FMINRES"
PetscErrorCode KSPSetUp_FMINRES(KSP ksp)
{
  PetscErrorCode ierr;

  PetscFunctionBegin;
  /*
  if (ksp->pc_side == PC_RIGHT) SETERRQ(PetscObjectComm((PetscObject)ksp),PETSC_ERR_SUP,"No right preconditioning for KSPFMINRES");
  else if (ksp->pc_side == PC_SYMMETRIC) SETERRQ(PetscObjectComm((PetscObject)ksp),PETSC_ERR_SUP,"No symmetric preconditioning for KSPFMINRES");
   */
...
#undef __FUNCT__
#define __FUNCT__ "KSPSolve_FMINRES"
PetscErrorCode  KSPSolve_FMINRES(KSP ksp)
{
  PetscErrorCode ierr;
  PetscInt       i;
  ...
  Mat            Amat,Pmat;
  KSP_FMINRES     *fminres = (KSP_FMINRES*)ksp->data;
  ...
  if (PetscRealPart(dp) < fminres->haptol) {
...
/*MC
     KSPFMINRES - This code implements the FMINRES (Minimum Residual) method.
...
M*/
#undef __FUNCT__
#define __FUNCT__ "KSPCreate_FMINRES"
PETSC_EXTERN PetscErrorCode KSPCreate_FMINRES(KSP ksp)
{
  KSP_FMINRES     *fminres;
  PetscErrorCode ierr;
  ...
  */
  ksp->ops->setup          = KSPSetUp_FMINRES;
  ksp->ops->solve          = KSPSolve_FMINRES;
