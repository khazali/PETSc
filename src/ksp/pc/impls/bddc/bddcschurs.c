...
PetscErrorCode PCBDDCSubSchursSetUp(PCBDDCSubSchurs sub_schurs, Mat Ain, Mat Sin, PetscBool exact_schur, PetscInt xadj[], PetscInt adjncy[], PetscInt nlayers, Vec scaling, PetscBool compute_Stilda, PetscBool reuse_solvers, PetscBool benign_trick, PetscInt benign_n, PetscInt benign_p0_lidx[], IS benign_zerodiag_subs[], Mat change, IS change_primal)
{
  ...
  ierr = PetscObjectCompose((PetscObject)sub_schurs->S_Ej_all,"_size_blocks",(PetscObject)is_I_layer);CHKERRQ(ierr);
  ...
  if (compute_Stilda) {
    ierr = PetscObjectCompose((PetscObject)sub_schurs->sum_S_Ej_tilda_all,"_size_blocks",(PetscObject)is_I_layer);CHKERRQ(ierr);
    ...
    if (deluxe) {
      ierr = PetscObjectCompose((PetscObject)sub_schurs->sum_S_Ej_inv_all,"_size_blocks",(PetscObject)is_I_layer);CHKERRQ(ierr);
