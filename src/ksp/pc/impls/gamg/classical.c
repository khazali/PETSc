...
PetscErrorCode PCGAMGClassicalInjection(PC pc,Mat P,VecScatter *inj)
{
  ...
  ierr = PetscMalloc1(fn,&cmap);CHKERRQ(ierr);
  ierr = PetscMalloc1(fn,&fval);CHKERRQ(ierr);
  ierr = PetscMalloc1(fn,&fmap);CHKERRQ(ierr);
  j=0;
...
PetscErrorCode PCGAMGClassicalBootstrapProlongator(PC pc,const Mat A,Mat *aP,PetscInt nv,Vec *vs)
{
  ...
  ierr = PetscMalloc1(nv,&wts);CHKERRQ(ierr);
  ...
  ierr = PetscMalloc6(ncolsmax*ncolsmax,&a,
                      ncolsmax,&b,
                      12*ncolsmax,&work,
                      ncolsmax,&s,
                      ncolsmax,&wvcols,
                      ncolsmax,&wicols);CHKERRQ(ierr);
#if defined(PETSC_USE_COMPLEX)
  ierr = PetscMalloc1(ncolsmax,&rwork);CHKERRQ(ierr);
#endif
...
PetscErrorCode PCGAMGBootstrap_Classical(PC pc,PetscInt nlevels,Mat *A,Mat *P)
{
  ...
  ierr = PetscMalloc1(nlevels,&vs);CHKERRQ(ierr);
  for (i=0;i<nlevels;i++) {
