...
/*
    ...
    A matrix free version of the burgers_spectral.c code. Not yet tested in parallel
...
extern PetscErrorCode RHSFunctionPETSC(TS,PetscReal,Vec,Vec,void*);
...
int main(int argc,char **argv)
{
  ...
  PetscErrorCode ierr;
  PetscInt       i, xs, xm, ind, j, lenglob,nn,m;
  ...
  appctx.param.N    = 15;  /* order of the spectral element */
  appctx.param.E    = 5;  /* number of elements */
...
/////////////////////////Temporary testing for jacobian

  Vec   ref, wrk_vec, jac, vec_jac, vec_rhs, temp, vec_trans, ref2, vec_rhs2;
  PetscScalar vareps;
  ...
  ierr = VecDuplicate(appctx.dat.ic,&ref2);CHKERRQ(ierr);
  ierr = VecDuplicate(appctx.dat.ic,&vec_rhs2);CHKERRQ(ierr);
  ...
  vareps = 1e-05;
    ...
    //PetscSNPrintf(var,sizeof(var),"rhs2(:,%d)",its);
    //ierr = PetscObjectSetName((PetscObject)vec_rhs2,var);
    //ierr = VecView(vec_rhs2,viewfile);CHKERRQ(ierr);
  ...
  xm=xm/(Nl-1);
   ...
   for (ix=xs; ix<xs+xm; ix++) 
     {for (jx=0; jx<Nl; jx++) 
        ...
        for (jy=0; jy<Nl; jy++) 
        {indy=ix*(Nl-1)+jy;
         outl[indx] += appctx->param.mu*2.0/appctx->param.Le*stiff[jy][jx]*ul[indy];//+grad[jx][jy]*ul[indy]*ul[indx];
        }
       }
     }
...
#undef __FUNCT__
#define __FUNCT__ "RHSFunctionPETSC"
PetscErrorCode RHSFunctionPETSC(TS ts,PetscReal t,Vec globalin,Vec globalout,void *ctx)
{
  PetscErrorCode ierr;
  AppCtx          *appctx = (AppCtx*)ctx;  
