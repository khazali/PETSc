...
The PETSc AMG framework provides one basic aggregation AMG method (\trl{-pc_gamg_type agg}) and the popular variant smoothed aggregation (\trl{-pc_gamg_agg_nsmooths 1}).
...
\findex{-pc_use_amat} One sets options for the subpreconditioners with the
...
The method SNESNEWTONLS (\trl{-snes_type} \trl{newtonls}) provides a line
...
\cite{upshot} \sindex{Jumppshot} to visualize PETSc events.
...
\section{Evaluating Residuals} \sindex{Residual Evaulation}
