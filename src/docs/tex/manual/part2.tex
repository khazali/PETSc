...
PETSc implements six different LMVM matrices listed in the Table~\ref{tab_matlmvmimpl}. They can be created using 
...
\lstinline{-mat_lmvm_num_vecs} option. Restricted Broyden Family, DFP and BFGS methods additionally implement scalar 
and diagonal scaling of the initial Jacobian which can be selected via \lstinline{-mat_lmvm_scale_type <none,scalar,diagonal>}.
...
The parameters for the eigen estimator can be set with the prefix``gamg_est_".
...
\href{http://www.mcs.anl.gov/petsc/petsc-current/src/ksp/ksp/examples/tutorials/ex10.c.html}{\trl{${PETSC_DIR}/src/ksp/ksp/examples/tutorials/ex10.c}}.
The code loads a
