...
int main( int argc, char **argv )
{
  ...
  */
  ...
  ierr = MatCreateAIJ(MPI_COMM_WORLD,m,m,N,N,1,NULL,1,NULL,&(user.B));CHKERRQ(ierr);
  ierr = MatCreateAIJ(MPI_COMM_WORLD,m,m,N,N,1,NULL,1,NULL,&(user.Bt));CHKERRQ(ierr);
  ...
  if (reason <= 0){
    ierr = PetscPrintf(PETSC_COMM_WORLD,"Try a different SNES method, adjust some parameters, or check the function evaluation routines\n");CHKERRQ(ierr);
  }
  /* Free SNES data structures */
