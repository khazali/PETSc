...
static PetscScalar FACTOR_A=1.0;//=1.0e+01;
static PetscScalar FACTOR_B=1.0;//=2.0e-05;
...
int main(int argc,char **argv)
{
  ...
  ierr = PetscOptionsReal("-factor_a","Factor for the first equation","<1.0>",FACTOR_A,&FACTOR_A,NULL);CHKERRQ(ierr);
  ierr = PetscOptionsReal("-factor_b","Factor for the second equation","<1.0>",FACTOR_B,&FACTOR_B,NULL);CHKERRQ(ierr);
  ...
  */
  ...
  ierr = PetscPrintf(PETSC_COMM_WORLD,"factor a = %g\n",FACTOR_A);CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD,"factor b = %g\n",FACTOR_B);CHKERRQ(ierr);

  ierr = PetscPrintf(PETSC_COMM_WORLD,"Number of SNES iterations = %D\n",its);CHKERRQ(ierr);

  ierr  = VecGetArray(x,&xx);CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD,"x(0) = %g\n",xx[0]);CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD,"x(1) = %g\n",xx[1]);CHKERRQ(ierr);
  ierr  = VecRestoreArray(x,&xx);CHKERRQ(ierr);
...
PetscErrorCode FormFunction1(SNES snes,Vec x,Vec f,void *ctx)
{
  ...
  /* Compute function */
  ff[0] = FACTOR_A*(xx[0]*xx[0] + xx[0]*xx[1] - 3.0);
  ff[1] = FACTOR_B*(xx[0]*xx[1] + xx[1]*xx[1] - 6.0);
  ...
  /*
     Compute Jacobian entries and insert into matrix.
      - Since this is such a small problem, we set all entries for
        the matrix at once.
  */
  A[0]  = FACTOR_A*(2.0*xx[0] + xx[1]); A[1] = FACTOR_A*xx[0];
  A[2]  = FACTOR_B*xx[1]; A[3] = FACTOR_B*(xx[0] + 2.0*xx[1]);
