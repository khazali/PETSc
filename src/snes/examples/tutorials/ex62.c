...
/*TEST
  ...
  test:
    ...
    filter: grep -v "variant HERMITIAN" | sed -e "s/linear solver iterations=5[0-9]/linear solver iterations=50/g" | sed -e "s/linear solver iterations=4[0-9]/linear solver iterations=50/g"
    nsize: 5
