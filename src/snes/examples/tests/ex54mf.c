...
PetscErrorCode ShellMatMult(Mat,Vec,Vec);
...
int main(int argc, char **argv)
{
  ...
  PetscInt                   its;
  PetscInt                   Mlocal,Nlocal,Mglobal,Nglobal;
  ...
  ierr = MatGetSize(J,&Mglobal,&Nglobal);CHKERRQ(ierr);
  ierr = MatGetLocalSize(J,&Mlocal,&Nlocal);CHKERRQ(ierr);
  ierr = MatCreateShell(PETSC_COMM_WORLD,Mlocal,Nlocal,Mglobal,Nglobal,(void*)&J,&Jshell);CHKERRQ(ierr);
  ierr = MatShellSetOperation(Jshell,MATOP_MULT,(void(*)(void))ShellMatMult);CHKERRQ(ierr);
  ...
  ierr = SNESSetJacobian(snes,Jshell,Jshell,FormJacobian,(void*)&user);CHKERRQ(ierr);
  ...
  ierr = MatDestroy(&Jshell);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "ShellMatMult"
PetscErrorCode ShellMatMult(Mat Shell,Vec X,Vec Y)
{
  PetscErrorCode ierr;
  void           *shctx;
  Mat            *J;
  ierr = MatShellGetContext(Shell,&shctx);CHKERRQ(ierr);
  J = (Mat*)shctx;
...
PetscErrorCode FormJacobian(SNES snes,Vec X,Mat Jshell,Mat B,void *ctx)
{
  PetscErrorCode   ierr;
  AppCtx           *user  =(AppCtx*)ctx;
  void             *shctx;
  ...
  PetscFunctionBeginUser;
  ierr = MatShellGetContext(Jshell,&shctx);CHKERRQ(ierr);
  J = (Mat*)shctx;
