...
#undef __FUNCT__
#define __FUNCT__ "MatColoringApply_Minpack"
PETSC_EXTERN PetscErrorCode MatColoringApply_Minpack(MatColoring mc,ISColoring *iscoloring)
{
  PetscErrorCode  ierr;
  PetscInt        *list,*work,*seq,clique,*coloring,n;
...
PETSC_EXTERN PetscErrorCode MatColoringCreate_Minpack(MatColoring mc)
{
    PetscFunctionBegin;
    mc->dist                = 2;
    mc->weight_type         = MAT_COLORING_WEIGHT_SL;
    mc->data                = NULL;
    mc->ops->apply          = MatColoringApply_Minpack;
    mc->ops->view           = NULL;
    mc->ops->destroy        = NULL;
