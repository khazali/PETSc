...
typedef struct{
  ...
  IS Rows, Cols;
  IS MaskedRows,MaskedCols; /* complement of index sets */
...
PetscErrorCode MatSMFResetRowColumn(Mat mat,IS Rows,IS Cols){
  ...
  ierr = ISDestroy(&ctx->MaskedRows);CHKERRQ(ierr);
  ierr = ISDestroy(&ctx->MaskedCols);CHKERRQ(ierr);
...
PetscErrorCode MatMult_SMF(Mat mat,Vec a,Vec y)
{
  ...
  ierr = VecISSet(ctx->VR,ctx->MaskedCols,0.0);CHKERRQ(ierr);
  ierr = MatMult(ctx->A,ctx->VR,y);CHKERRQ(ierr);
  ierr = VecISSet(y,ctx->MaskedRows,0.0);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatMultTranspose_SMF(Mat mat,Vec a,Vec y)
{
  ...
  ierr = VecISSet(ctx->VC,ctx->MaskedRows,0.0);CHKERRQ(ierr);
  ierr = MatMultTranspose(ctx->A,ctx->VC,y);CHKERRQ(ierr);
  ierr = VecISSet(y,ctx->MaskedCols,0.0);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatDestroy_SMF(Mat mat)
{
  ...
  ierr = VecDestroy(&ctx->VR);CHKERRQ(ierr);
...
PetscErrorCode MatCreateSubMatrixFree(Mat mat,IS Rows, IS Cols, Mat *J)
{
  ...
  ierr = MatGetOwnershipRange(mat,&lo,&hi);CHKERRQ(ierr);
  ierr = MPI_Comm_size(comm,&size);
  if (size == 1) {
    ierr = VecCreateSeq(comm,m,&ctx->VR);CHKERRQ(ierr);
    ierr = VecCreateSeq(comm,n,&ctx->VC);CHKERRQ(ierr);
  } else {
    ierr = VecCreateMPI(comm,mloc,m,&ctx->VR);CHKERRQ(ierr);
  ...
  ctx->Rows = Rows;
  ctx->Cols = Cols;
  ierr = ISComplement(Rows,lo,hi,&ctx->MaskedRows);CHKERRQ(ierr);
  if (ctx->Rows == ctx->Cols) {
    ctx->MaskedCols = ctx->MaskedRows;
    PetscObjectReference((PetscObject)ctx->MaskedRows);
  }
