...
PetscErrorCode  MatSeqBAIJSetPreallocation_SeqBAIJ(Mat B,PetscInt bs,PetscInt nz,PetscInt *nnz)
{
  ...
  PetscBool      flg = PETSC_FALSE,skipallocation = PETSC_FALSE,realalloc = PETSC_FALSE,baij_loop=PETSC_FALSE;
  ...
  ierr = PetscOptionsBool("-mat_baij_loop","Use loops instead of manually unrolled implementations",NULL,baij_loop,&baij_loop,NULL);CHKERRQ(ierr);
  ...
  if (!flg) {
    ...
    case 3:
      ...
      break;
    case 4:
      B->ops->mult    = baij_loop ? MatMult_SeqBAIJ_4_loop : MatMult_SeqBAIJ_4;
