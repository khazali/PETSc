...
int main(int argc,char **args)
{
  ...
  PetscBool      flg,flgB,isElemental=PETSC_FALSE,isDense,isAij,isSbaij;
  PetscScalar    one = 1.0,alpha,beta;
  ...
  if (!isElemental) {
    ...
    El::HermitianEigCtrl<PetscElemScalar> ctrl;
    El::DistMatrix<PetscElemScalar> *Aee,*Bee;
    ierr = MatGetElementalMat(Ae,(void**)&Aee);CHKERRQ(ierr);
    ierr = MatGetElementalMat(Be,(void**)&Bee);CHKERRQ(ierr);
    ...
    ierr = MatCreateElementalWithEl(Xee,&Xe);CHKERRQ(ierr);
    //if (!rank) printf(" Petsc Eigenvectors:\n");
    //ierr = MatView(Xe,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
