...
#include <petscmat.h>
static PetscErrorCode MatGetOrdering_myordering(Mat,MatOrderingType,IS*,IS*);
...
static PetscErrorCode MatGetOrdering_myordering(Mat mat,MatOrderingType type,IS *irow,IS *icol)
{
  PetscErrorCode ierr;
  PetscInt       n,i,*ii;
  PetscBool      done;
