  ...
  stage('Examples') {
    steps {
      parallel (
        "arch-c-exodus-dbg-builder" : {
          node('frog') {
            sh "make PETSC_ARCH=arch-c-exodus-dbg-builder PETSC_DIR=${WORKSPACE} -f gmakefile test search='tao_unconstrained%'"
          }
        },
        "arch-linux-pkgs-dbg-ftn-interfaces" : {
          node('n-gage') {
            sh "make PETSC_ARCH=arch-linux-pkgs-dbg-ftn-interfaces PETSC_DIR=${WORKSPACE} -f gmakefile test search='tao_unconstrained%'"
          }
        }
      )
    }
